...
User Function STX099IR()
    ...
    DEFINE DIALOG oDlg TITLE 'Rotina de Ajuste de IRR do lançamento do documento fiscal' PIXEL STYLE nOR(  WS_VISIBLE ,  WS_POPUP )
...
Static Function cria_pg1(oPanel)
    ...
    oSay2	:= TSay():New(025,010,{|| 'A rotina a seguir irá disponibilizar os produtos da Nota Fiscal para os devidos ajustesde IRR no lançamento.'},oPanel,,,,,,.T.,,,400,20)
...
Static Function cria_pg2(oPanel)
	...
	Local aColsEx		:= {}
	Local aFieldFill	:= {}
	Local aFields		:= {"D1_ITEM","D1_COD","D1_XDESCR","D1_QUANT","D1_VUNIT","D1_TOTAL","D1_BASEIRR","D1_ALIQIRR","D1_VALIRR","NEW_BASEIRR","NEW_ALIQ_IRR","NEW_VAL_IRR","PIS_IRRF"}
	Local aAlterFields	:= {"NEW_BASEIRR","NEW_ALIQ_IRR"}
	Local cQuery		:= ""
	...
	For nX := 1 to Len(aFields)
        cDicCampo := If(aFields[nX] == "NEW_ALIQ_IRR","D1_ALIQIRR",If(aFields[nX] == "NEW_VAL_IRR","D1_VALIRR",If(aFields[nX] == "PIS_IRRF","D1_VALIRR",If(aFields[nX] == "NEW_BASEIRR","D1_BASEIRR",aFields[nX]))))

        Aadd(aHeaderEx, {If(aFields[nX]=="NEW_ALIQ_IRR","Aliq Correta",If(aFields[nX]=="NEW_VAL_IRR","Val Correto",;
                                If(aFields[nX]=="PIS_IRRF","Diferença",If(aFields[nX]=="NEW_BASEIRR","Base IRRF Correta",GetSX3Cache(cDicCampo, "X3_TITULO"))))),;
            If(aFields[nX] $ "NEW_ALIQ_IRR/NEW_VAL_IRR/PIS_IRRF/NEW_BASEIRR",aFields[nX],GetSX3Cache(cDicCampo, "X3_CAMPO")),;
            GetSX3Cache(cDicCampo, "X3_PICTURE"),GetSX3Cache(cDicCampo, "X3_TAMANHO"),GetSX3Cache(cDicCampo, "X3_DECIMAL"),;
            If(aFields[nX] $ "NEW_ALIQ_IRR/NEW_BASEIRR","Eval(bValIRR)",GetSX3Cache(cDicCampo, "X3_VALID")),;
            If(aFields[nX] $ "NEW_ALIQ_IRR/NEW_BASEIRR",NIL,GetSX3Cache(cDicCampo, "X3_USADO")),;
	...
	if !(cAlias)->(Eof())
		While !(cAlias)->(Eof())
			For nX := 1 to Len(aFields)
                If aFields[nX] $ "NEW_ALIQ_IRR"
                    Aadd(aFieldFill, (cAlias)->D1_ALIQIRR)
                ElseIf aFields[nX] $ "NEW_VAL_IRR"
				    Aadd(aFieldFill, (cAlias)->D1_VALIRR)
                ElseIf aFields[nX] $ "NEW_BASEIRR"
				    Aadd(aFieldFill, (cAlias)->D1_BASEIRR)
                ElseIf aFields[nX] $ "PIS_IRRF"
				    Aadd(aFieldFill, 0)
                Else
			...
			aFieldFill := {}
            nVlrIRR  += (cAlias)->D1_VALIRR
            nVlrIRRCorr  += (cAlias)->D1_VALIRR
			(cAlias)->(dbSkip())
		EndDo
	Else
		For nX := 1 to Len(aFields)
            If aFields[nX] $ "NEW_ALIQ_IRR/NEW_VAL_IRR/PIS_IRRF/NEW_BASEIRR"
                Aadd(aFieldFill, 0)
            Else
    ...
    oSay2       := TSay():New(162,004,{|| 'Valor IRRF Informado:'},oPanel,,,,,,.T.,,,300,20)
    oGetDOrig   := TGet():New(169,004, bSetGet(nVlrIRR),oPanel, 080, 010, "@E 999,999,999.99", {|| .F.},,,,,, .T.,,, {||  .F.},,, {|| .F.}, .F., .F.,, "nVlrIRR",,,, .T.)
    oSay3	    := TSay():New(182,004,{|| 'Valor IRRF Corrigido:'},oPanel,,,,,,.T.,,,300,20)
    oGetIRR     := TGet():New(189,004, bSetGet(nVlrIRRCorr),oPanel, 080, 010, "@E 999,999,999.99", {|| .F.},,,,,, .T.,,, {||  .F.},,, {|| .F.}, .F., .F.,, "nVlrIRRCorr",,,, .T.)
...
Static Function processa_pg2()
    ...
    If lReturn
        If nVlrProds == nVlrPCorr .And. nVlrIRR == nVlrIRRCorr
...
Static Function fVldImp()
    ...
    Local nX
    Local lRet      := .T.
	Local nPosVIRR	:= aScan(oMSGridImp:aHeader,{|x| AllTrim(x[2]) == 'D1_VALIRR'})
    Local nPosNVIRR := aScan(oMSGridImp:aHeader,{|x| AllTrim(x[2]) == 'NEW_VAL_IRR'})
    Local nPosNAIRR := aScan(oMSGridImp:aHeader,{|x| AllTrim(x[2]) == 'NEW_ALIQ_IRR'})
    Local nPosNBIRR := aScan(oMSGridImp:aHeader,{|x| AllTrim(x[2]) == 'NEW_BASEIRR'})
	Local nPosPISIRR:= aScan(oMSGridImp:aHeader,{|x| AllTrim(x[2]) == 'PIS_IRRF'})
    Local nAliqIRR  := If(ValType(M->NEW_ALIQ_IRR) <> "U", M->NEW_ALIQ_IRR,oMSGridImp:aCols[oMSGridImp:nAt][nPosNAIRR])
    Local nValorBase:= If(ValType(M->NEW_BASEIRR) <> "U", M->NEW_BASEIRR, oMSGridImp:aCols[oMSGridImp:nAt][nPosNBIRR])
    
    If lRet .And. Valtype(oMSGridImp) <> "U"

        If ValType(M->NEW_ALIQ_IRR) <> "U" .And. M->NEW_ALIQ_IRR < 0
            MsgStop("O valor informado deve ser maior do que zero!", "Valor informado inválido")
            lRet    := .F.
        ElseIf ValType(M->NEW_BASEIRR) <> "U" .And. M->NEW_BASEIRR < 0
            MsgStop("O valor informado deve ser maior do que zero!", "Valor informado inválido")
            lRet    := .F.
        Else
            //oMSGridImp:aCols[oMSGridImp:nAt][nPosNVIRR] := oMSGridImp:aCols[oMSGridImp:nAt][nPosNBIRR] * M->NEW_ALIQ_IRR / 100
            oMSGridImp:aCols[oMSGridImp:nAt][nPosNVIRR] := nValorBase * nAliqIRR / 100
            oMSGridImp:aCols[oMSGridImp:nAt][nPosPISIRR]:= oMSGridImp:aCols[oMSGridImp:nAt][nPosNVIRR] - oMSGridImp:aCols[oMSGridImp:nAt][nPosVIRR]

            nVlrIRRCorr := 0

            For nX := 1 To Len(oMSGridImp:aCols)
                nVlrIRRCorr  += oMSGridImp:aCols[nX][nPosVIRR] + oMSGridImp:aCols[nX][nPosPISIRR]
            Next nX
...
Static Function fAjustaNF(oProcess)
	...
	Local nPosDifDf	:= aScan(oMSGridImp:aHeader,{|x| AllTrim(x[2]) == 'PIS_IRRF'})
    Local nValorTot := 0
    Local aArea		:= GetArea()
    Local lReturn 	:= .T.
    LOCAL oAjusteNF	:= NIL
    ...
    oProcess:IncRegua1("Processando ajustes relativos a impostos - IRR...")

    If nVlrIRR <> nVlrIRRCorr

        //IncProc("Realizando ajustes relativos ao IRR")
        ...
        Begin Transaction
            ...
            For nX := 1 To Len(oMSGridImp:aCols)
                ...
                // Verifico se há ajuste de IRR para realizar
                If oMSGridImp:aCols[nX][nPosDifDf] <> 0
                    ...
                    If SD1->(DBSeek(SF1->(F1_FILIAL+F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA)+oMSGridImp:aCols[nX][nPosCod]+oMSGridImp:aCols[nX][nPosItem]))
                        ...
                        If !Empty(SD1->D1_TES) .And. SF4->(DBSeek(xFilial("SF4")+SD1->D1_TES))
                            ...
                            lReturn:= oAjusteNF:AtuLivro99(oMSGridImp, nX, "IRRF")
                            ...
                            If !Empty(SF1->F1_DTLANC)
                                ...
                                lReturn:= oAjusteNF:AtuCont99(oMSGridImp, nX, "IRRF")
                                //lReturn:= U_AtuCont99(oMSGridImp, nX, "IRRF")
            ...
            If nValorTot <> 0
                ...
                lReturn:= oAjusteNF:AtuCtPag99(nValorTot, "IRRF")
                //lReturn:= U_AtuCtPag99(nValorTot, "IRRF")
