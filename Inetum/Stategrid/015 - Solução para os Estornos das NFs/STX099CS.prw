...
User Function STX099CS()
    ...
    DEFINE DIALOG oDlg TITLE 'Rotina de Ajuste de CSLL do lançamento do documento fiscal' PIXEL STYLE nOR(  WS_VISIBLE ,  WS_POPUP )
...
Static Function cria_pg1(oPanel)
    ...
    oSay2	:= TSay():New(025,010,{|| 'A rotina a seguir irá disponibilizar os produtos da Nota Fiscal para os devidos ajustesde CSLL no lançamento.'},oPanel,,,,,,.T.,,,400,20)
...
Static Function cria_pg2(oPanel)
	...
	For nX := 1 to Len(aFields)
        ...
        Aadd(aHeaderEx, {If(aFields[nX]=="NEW_ALIQ_CSLL","Aliq Correta",If(aFields[nX]=="NEW_VAL_CSLL","Val Correto",;
                                If(aFields[nX]=="PIS_CSLL","Diferença",If(aFields[nX]=="NEW_BASECSLL","Base CSLL Correta",GetSX3Cache(cDicCampo, "X3_TITULO"))))),;
    ...
    If nVlrCSLL <> nVlrCSLLCorr
        ...
        Begin Transaction
            ...
            For nX := 1 To Len(oMSGridImp:aCols)
                ...
                // Verifico se há ajuste de CSLL para realizar
