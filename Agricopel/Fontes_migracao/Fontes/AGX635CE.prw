		...
		For nCountPara := 1 To Len(aEmpPara)
				...
				RPCSetType(3)
			  	RPCSetEnv(cEmpPara, cFilialPara)
					...
					cCapaCTE := GetNextAlias()
					DbUseArea(.T., Nil, cArqTmp, (cCapaCTE))
...
Static Function SelectCTE(nEmpOrigem,nFilOrigem)
   ...
   //	cQuery += " ,STG_COM_CONFRE_Etr_Codigo AS DBENT "+CHR(13) 
	...
	cQuery += " ,COM_CONFRE_TipoFrete  AS F1_COND    "+CHR(13)
	...
	cQuery += " ,COM_CONFRE_AICMS    AS AICMS      "+CHR(13)
    ...
    //cQuery += " AND COM_CONFRE_Numero = 5291120 " //OK 
...
Static Function InserirCTE(cCapaCTE)
	...
	Local lRegOK 	:= .T. 
	Local cFil   	:= '' 
	Local cENTouSAI := 'E' //Variavel para controlar se é de nota de entrada ou saída
	...
	While (cCapaCTE)->(!eof())
		...
		If !SA2->(DbSeek(xFilial("SA2")+(cCapaCTE)->(CNPJ_CPF)))
			...
			//Inclui fornecedor  
	   		U_AGX635CF((cCapaCTE)->(CNPJ_CPF),'SA2')
		...
		//Dbselectarea('SF1')
		//DbsetOrder(1)           //DOC + SERIE + FORNECE +LOJA + TIPO
		//If Dbseek(xfilial('SF1')+_cDoc+_cSerie+SA2->A2_COD+SA2->A2_LOJA )
	    ...
	    If  cENTouSAI == 'E'
			...
			BEGIN TRANSACTION //Begintran()	 
				...
				aAdd(aCabec116,{"F1_BASERET",(cCapaCTE)->(F1_BASEICM)})						   			// Valor da base de calculo do ICMS retido
				aAdd(aCabec116,{"F1_ICMRET",(cCapaCTE)->(F1_VALICM)})							 		// Valor do ICMS retido
		...
		Else //Cte de Entrada + NF de Saída
			...
			BEGIN TRANSACTION     
				...
				Reclock('SF1',.T.)  
					...
					SF1->F1_COND    := '001'//",'001'})	
					SF1->F1_DUPL    := _cDoc 
					SF1->F1_PREFIXO := xFilial('SF1')+SUBSTR(ALLTRIM( (cCapaCTE)->(F1_SERIE) ),1,3)
		    	...
		    	dbselectarea('SB1')
				dbsetorder(1) 	
			    dbseek(xfilial('SB1')+'DES52111101' ) 
			    
			    BEGIN TRANSACTION        
			    Reclock('SD1',.T.)
			    	SD1->D1_FILIAL  := SF1->F1_FILIAL 
			    	SD1->D1_ITEM    := '0001' 
			    	SD1->D1_COD     := 'DES52111101' 
...
Static Function InserirNfe(xDocCab,xSerCab,xFornCab,xLojaCab)
	...
	While (cAliasNFE)->(!eof())  
		...
		If !SA2->(DbSeek(xFilial("SA2")+(cAliasNFE)->(CNPJ_CPF)))
			...
			If !lReproc
        		...
        		AADD(aLogs,{;
						...
						{'ZDB_EMP'	  ,cEmpant},;
						{'ZDB_FILIAL' ,cFilAnt},;
						{'ZDB_DBCHAV' ,(cAliasNFE)->(F1_DOC)+'+'+(cAliasNFE)->(F1_SERIE)+'+'+(cAliasNFE)->(DBFORN)},; 
						{'ZDB_TAB' 	  ,'SA2'},; 
						{'ZDB_INDICE' ,3},; 
						{'ZDB_TIPOWF' ,4},; 
        ...
        aTam    := {};aTam    := TamSX3("F1_SERIE")	;  _cSerie2 := PADR(alltrim((cAliasNFE)->(F1_SERIE)),aTam[1],' ') 
		...
		If dbseek(xfilial('SF1')+_cDoc2+_cSerie2+SA2->A2_COD+SA2->A2_LOJA) 
			...
			AAdd(aItens116, {{"PRIMARYKEY", SubStr(SF1->&(IndexKey()), FWSizeFilial() + 3)}})  
		...
		Else 
			...
			If !lReproc
        		...
        		AADD(aLogs,{;
						...
						{'ZDB_EMP'	  ,cEmpant},;
						{'ZDB_FILIAL' ,cFilAnt},;
						{'ZDB_DBCHAV' ,_cDoc2+'+'+_cSerie2+'+'+SA2->A2_COD+'+'+SA2->A2_LOJA},; 
						{'ZDB_TAB' 	  ,'SF1'},; 
						{'ZDB_INDICE' ,1	},; 
						{'ZDB_TIPOWF' ,4},; 
	    ...
	    ddatabase := ddataOLD 
	   	...
	   	if lMsErroAuto
        		...
        		AADD(aLogs,{;
						{'ZDB_DBEMP'  ,(cCapaCTE)->(DBEMP)},;
						{'ZDB_DBFIL'  ,(cCapaCTE)->(DBFIL)}								,;
						{'ZDB_MSG'	  ,	'Erro MATA116: '+(cCapaCTE)->(F1_DOC)+'-'+(cCapaCTE)->(F1_SERIE)+" Verifique se estoque está aberto e Doc contido no Cte esta lancado."},;
						{'ZDB_DATA'	  ,ddatabase},;
						{'ZDB_HORA'	  ,time()},;
						{'ZDB_EMP'	  ,cEmpant},;
						{'ZDB_FILIAL' ,cFilAnt},;
...
Static Function ENTouSAI()
	...
	If Select('ENTouSAI') <> 0
		dbSelectArea('ENTouSAI')
		(ENTouSAI)->(dbCloseArea())
	Endif

	TCQuery cQueryES NEW ALIAS "ENTouSAI" 
