...
PRIVATE lMarcCte	:= .T.
	...
	ElseIf !aAcesso[3]
		...
		Return
	ElseIf !Empty(aAcesso[4]) .and. ((CToD(aAcesso[4]) - Date()) < 5)
	...
	dbSelectArea("SX3")
	SX3->( dbSetOrder(2) )
	If !(SX3->( dbSeek(_cCmp1+"_FILIAL") ) .And. SX3->( dbSeek(_cCmp1+"_CHAVE") ) .And. SX3->( dbSeek(_cCmp1+"_XML") ))
...
// Baixar os XMLs do e-mail.
	IF GETNEWPAR("MV_IMPMAIL", .F.)    
	    DBSELECTAREA('SX2') // Cria registro na SX2 se não existir
    	DBSETORDER(1)
	    IF !DBSEEK('ZZ4')
    		RECLOCK("SX2",.T.)
    		SX2->X2_CHAVE 	:= "ZZ4"
	    	SX2->X2_PATH	:= "\SIGAADV\"
    		SX2->X2_ARQUIVO := "ZZ4010"
    		SX2->X2_NOME 	:= "E-MAILS IMPORTADOR DE E-MAIL"
	    	SX2->X2_NOMESPA := "E-MAILS IMPORTADOR DE E-MAIL"
    		SX2->X2_NOMEENG := "E-MAILS IMPORTADOR DE E-MAIL"
    		SX2->X2_MODO 	:= "C"
	    	SX2->X2_MODOUN 	:= "C"
    		SX2->X2_MODOEMP := "C"
    		SX2->(MSUNLOCK())  
    		...
    		SIX->INDICE		:= "ZZ4"
    		SIX->ORDEM		:= "1"
	    	SIX->CHAVE		:= "ZZ4_FILIAL+ZZ4_EMP+ZZ4_FIL"
    		SIX->DESCRICAO	:= "Empresa+Filial Email"
    		SIX->DESCSPA	:= "Empresa+Filial Email"
	    	SIX->DESCENG	:= "Empresa+Filial Email"
    		SIX->PROPRI		:= "U"
    		SIX->SHOWPESQ	:= "S"
	    	SIX->(MSUNLOCK())
	    	...
	    	SIX->INDICE		:= "ZZ4"
    		SIX->ORDEM		:= "2"
    		SIX->CHAVE		:= "ZZ4_FILIAL+ZZ4_EMP+ZZ4_FIL+ZZ4_EMAIL"
	    	SIX->DESCRICAO	:= "Empresa+Filial Email+E-MAIL"
    		SIX->DESCSPA	:= "Empresa+Filial Email+E-MAIL"
    		SIX->DESCENG	:= "Empresa+Filial Email+E-MAIL"
	    	SIX->PROPRI		:= "U"
    		SIX->SHOWPESQ	:= "S"
    		SIX->(MSUNLOCK())
	    ENDIF
	...
	If TelaParam(.F.)           
		...
		IF GETNEWPAR("StflB7c2", .F.)   // Melhoria para pegar automaticamente o caminho das pastas, quando utilizado a integração com o e-mail - Thiago SLA - 22/06/2016
		...
		cArqTabFil := CriaTrab(aStruFil, .T.)
		dbUseArea(.T., __LocalDriver, cArqTabFil, cAliFil, .F., .F.)
		IndRegua(cAliFil, cArqTabFil, "ARQ",,,"")
		...
		DEFINE MSDIALOG oDlgMain FROM aSize[7],0 TO aSize[3], aSize[5] TITLE 'Importador XML' PIXEL
			...
			oPanelTot := tPanel():New(0,0,,oDlgMain,,,,,,655,10,.T.,.F.)
			oPanelTot :Align := CONTROL_ALIGN_ALLCLIENT

			/////////////////////////////////////  CONSTRUÇÃO DO FOLDER
			oTFolder := tfolder():new( 11,0,{"NF-e","CT-e","CC-e","Cancelamento","XML's Pendentes"},,oDlgMain,,,,.T.,,655,285,,.T.)
			...
			@021,325 MSGET cCCCTe  		SIZE 045,009 PIXEL OF oLayerCTe:GetWinPanel('CTE_INF', 'WIN_CTE_INF', 'TOP') WHEN .T. F3 "CTT" VALID IIf(!Empty(cNatCTe), ExistCPO("CTT", cCCCTe), .T.) PICTURE "@! XXXXXXXXXX" HASBUTTON
			...
			////// Valida se é cliente apenas de CTe
			If  aAcesso[5]
				oTFolder:HidePage(1)
				oTFolder:HidePage(3)
				oTFolder:HidePage(4)
...
Local nPos := 0
	...
	If cType=="PEN"
		...
		SX3->( dbSeek(_cTab4) )
		While !SX3->( Eof() ) .And. SX3->X3_ARQUIVO == _cTab4
			If SX3->X3_BROWSE == "S"
	...
	Else
		...
		While !SX3->( Eof() ) .And. SX3->X3_ARQUIVO == _cTab1
			If SX3->X3_BROWSE == "S" .And. AScan(aNotCol[nPos, 2], {|x| x == AllTrim(SX3->X3_CAMPO)}) == 0
				AAdd(aFields, AllTrim(SX3->X3_CAMPO))
			EndIf
			SX3->( dbSkip() )
		EndDo
			...
			DEFINE MSDIALOG oDlgNFeC FROM aSize[7],0 TO aSize[6],aSize[5] TITLE 'Importador - Nota Fiscal de Complemento de ' + cDesComp STYLE DS_MODALFRAME PIXEL
				...
				oLayerNFeC:AddWindow('NFEC_ITEM', 'WIN_NFEC_ITEM', "Itens da Nota Fiscal de Complemento de " + cDesComp + " a ser importada", 100, .F., .T.,, 'CENTER',)
					...
					While !SX3->( Eof() ) .And. SX3->X3_ARQUIVO == _cTab2
						...
						If !(AllTrim(SX3->X3_CAMPO) $ _cCmp2+"_FILIAL|"+_cCmp2+"_PEDIDO|"+_cCmp2+"_ITEMPC|"+_cCmp2+"_BASEIC|"+_cCmp2+"_PICM|"+_cCmp2+"_VALICM|"+_cCmp2+"_BASEIP|"+_cCmp2+"_IPI|"+_cCmp2+"_VALIPI|"+_cCmp2+"_QUANT1")
				...
				oLayerNFeC:AddWindow('NFEC_NFE', 'WIN_NFEC_NFE', "Itens das Notas Fiscais de origem (Entrada)", 100, .F., .T.,, 'MIDDLE',)
					...
					While !SX3->( Eof() ) .And. SX3->X3_ARQUIVO == "SD1"
						...
						If SX3->X3_CONTEXT # "V" .And. !(AllTrim(SX3->X3_CAMPO) $ "D1_FILIAL") .And. AScan(aNFsIni, {|x| x = AllTrim(SX3->X3_CAMPO)}) == 0
							AADD(aHeadNFs, {Trim(X3Titulo()), SX3->X3_CAMPO, SX3->X3_PICTURE, SX3->X3_TAMANHO, SX3->X3_DECIMAL, SX3->X3_VALID, SX3->X3_USADO, SX3->X3_TIPO, SX3->X3_F3, SX3->X3_CONTEXT})
						EndIf
						SX3->( dBSkip() )
			...
			DEFINE MSDIALOG oDlgNFeD FROM aSize[7],0 TO aSize[6],aSize[5] TITLE 'Importador - Nota Fiscal de ' + cDescDev STYLE DS_MODALFRAME PIXEL
				...
				oLayerNFeD:AddWindow('NFED_ITEM', 'WIN_NFED_ITEM', "Itens da Nota Fiscal de " + IIf(cCombo1=="D", "Devolução", "Beneficiamento") + " a ser importada", 100, .F., .T.,, 'CENTER',)
					...
					While !SX3->( Eof() ) .And. SX3->X3_ARQUIVO == _cTab2
						...
						If !(AllTrim(SX3->X3_CAMPO) $ _cCmp2+"_FILIAL|"+_cCmp2+"_PEDIDO|"+_cCmp2+"_ITEMPC|"+_cCmp2+"_BASEIC|"+_cCmp2+"_PICM|"+_cCmp2+"_VALICM|"+_cCmp2+"_BASEIP|"+_cCmp2+"_IPI|"+_cCmp2+"_VALIPI") //
							AADD(aHeadItem, {Trim(X3Titulo()), SX3->X3_CAMPO, SX3->X3_PICTURE, SX3->X3_TAMANHO, SX3->X3_DECIMAL, SX3->X3_VALID, SX3->X3_USADO, SX3->X3_TIPO, SX3->X3_F3, SX3->X3_CONTEXT})
						EndIf
						SX3->( dBSkip() )
					...
					For nI := 1 To Len(oXml:_NfeProc:_Nfe:_InfNfe:_det)
						...
						aColsItem[nI][4]  := IIf(!Empty(cProduto), Posicione("SB1", 1, xFilial("SB1") + cProduto, "B1_DESC"), Space(TamSX3("B1_DESC")[1])) // Descrição do Produto
						aColsItem[nI][5]  := oXml:_nfeProc:_NFe:_infNFe:_det[nI]:_prod:_uCom:Text // Unidade de Medida
						aColsItem[nI][6]  := Round(Val(oXml:_NfeProc:_Nfe:_InfNfe:_det[nI]:_Prod:_qCom:Text), nDecQtd) // Quantidade no XML
						aColsItem[nI][7]  := Round(Val(oXml:_NfeProc:_Nfe:_InfNfe:_det[nI]:_Prod:_qCom:Text), nDecQtd) // Quantidade da Nota de saída selecionada
						aColsItem[nI][8]  := Round(Val(oXml:_NfeProc:_Nfe:_InfNfe:_det[nI]:_Prod:_vUnCom:Text), nDecVal) // Valor unitário
						aColsItem[nI][9]  := Round(aColsItem[nI][7] * aColsItem[nI][8], nDecTot) // Valor total
						aColsItem[nI][10] := IIf(!Empty(cProduto), Posicione("SB1", 1, xFilial("SB1") + cProduto, "B1_TE"), Space(TamSX3("B1_TE")[1])) // Tes
						aColsItem[nI][11] := IIf(!Empty(aColsItem[nI][10]), IIf(GETMV("MV_ESTADO") == M->&(_cCmp1+"_EST"), "1", "2") + Substr(Posicione("SF4", 1, xFilial("SF4") + aColsItem[nI][10], "F4_CF"), 2, 3), "") // Código Fiscal
						aColsItem[nI][12] :=  cClasFis // Classificação Fiscal
				...
				oLayerNFeD:AddWindow('NFED_NFS', 'WIN_NFED_NFS', "Itens das Notas Fiscais de origem (Saída)", 100, .F., .T.,, 'MIDDLE',)
					...
					While !SX3->( Eof() ) .And. SX3->X3_ARQUIVO == "SD2"
						...
						If SX3->X3_CONTEXT # "V" .And. !(AllTrim(SX3->X3_CAMPO) $ "D2_FILIAL") .And. AScan(aNFsIni, {|x| x = AllTrim(SX3->X3_CAMPO)}) == 0
							AADD(aHeadNFs, {Trim(X3Titulo()), SX3->X3_CAMPO, SX3->X3_PICTURE, SX3->X3_TAMANHO, SX3->X3_DECIMAL, SX3->X3_VALID, SX3->X3_USADO, SX3->X3_TIPO, SX3->X3_F3, SX3->X3_CONTEXT})
						EndIf
...
Private lMsErroAuto    := .F.
	...
	For nItem := 1 To Len(aColsItem)
		...
		AAdd(aItens, {{"D1_ITEM"   , StrZero(nItem, 4)                                                 , Nil},;
	    	          ...
	    	          {"D1_ICMRET" , GDFieldGet(_cCmp2+"_ICMRET", nItem,, oGetDItem:aHeader, aColsItem)   , Nil},;
	    	          {"AUTDELETA" , "N"			                                                   , Nil};
	    	         })
	...
	Begin Sequence
			...
			For nX1 := 1 To Len(oXml:_nfeProc:_NFe:_infNFe:_det)
				...
				If Type("oXml:_nfeProc:_NFe:_infNFe:_det[" + cValToChar(nX1) + "]:_imposto:_ICMS") == "O"
					...
					If At("<ICMS60>", cStrXml) > 0  .or. At("<ICMSST>", cStrXml) > 0  //Nova TAG
						...
						//nvBCSTRet := Val(Substr(cStrXml, At("<vBCSTRet>", cStrXml) + 7, At("</vBCSTRet>", cStrXml) - (At("<vBCSTRet>", cStrXml) + 7) ))
						//nvICMSSTRet := Val(Substr(cStrXml, At("<vICMSSTRet>", cStrXml) + 9, At("</vICMSSTRet>", cStrXml) - (At("<vICMSSTRet>", cStrXml) + 9)))
						nValICM  := Val(Substr(cStrXml, At("<vICMS>", cStrXml) + 7, At("</vICMS>", cStrXml) - (At("<vICMS>", cStrXml) + 7)))
						nPmVast  := Val(Substr(cStrXml, At("<pMVAST>", cStrXml) + 8, At("</pMVAST>", cStrXml) - (At("<pMVAST>", cStrXml) + 8) ))
		...
		// montagem do aheader
			...
			While !SX3->( Eof() ) .AND. X3_ARQUIVO == _cTab2
				...
				If SX3->X3_CAMPO # _cCmp2+"_FILIAL"
					nUsado++
			...
			For nY := 1 to Len(aItens)
				...
				aCols[nY,27] := aItens[nY,21]//aItens[nY,20]
		...
		if lVldPc
			If !Empty(oGetD:aCols[nX,2])
				...
				If !SC7->(dbSeek(xFilial("SC7") + oGetD:aCols[nX,2] + oGetD:aCols[nX,3])) .and. !SC7->(dbSeek(xFilial("SC7") + oGetD:aCols[nX,2] + StrZero(val(oGetD:aCols[nX,3]),4) ))
					MsgStop("2Número do Pedido de Compra informado no item " + oGetD:aCols[nX,2] + " / " + StrZero(nX,4) + " não foi encontrado.","Aviso")
					_lPedC := .F.
					Return .F.
		...
		if !lPreNota 
			/*AAdd(aItemPC, {{"D1_ITEM"	, StrZero(nX,4)		, Nil},;
						...
						If oGetD:aCols[nX,26] > 0 .AND. SUBSTR(oGetD:aCols[nX,19],2,2) == '60'  .OR.;
							...
							AAdd(aItemPCLIN, {"D1_ALQNDES"	, oGetD:aCols[nX,25], Nil})
							AAdd(aItemPCLIN, {"D1_BASNDES"  , oGetD:aCols[nX,24]/*oGetD:aCols[nX,25]*/, Nil})
							AAdd(aItemPCLIN, {"D1_ICMNDES"  , oGetD:aCols[nX,26]/*oGetD:aCols[nX,27]*/, Nil})
						ELSE                                                                                
							AAdd(aItemPCLIN, {"D1_ALIQSOL"	, oGetD:aCols[nX,25], Nil})
...
// While (_cTab1)->(!EOF()) .AND. (_cTab1)->(&(_cCmp1+"_FILIAL")+&(_cCmp1+"_CODEMI")+&(_cCmp1+"_LOJEMI")+&(_cCmp1+"_DOC")+&(_cCmp1+"_SERIE")) == xFilial(_cTab1)+(_cTab1)->(&(_cCmp1+"_FORNEC")+&(_cCmp1+"_LOJA")+&(_cCmp1+"_DOC")+&(_cCmp1+"_SERIE"))
	...
	If !Empty((_cTab1)->&(_cCmp1+"_PEDIDO"))
		...
		aAdd(aTail(aRet),{"D1_ITEMPC", (_cTab1)->&(_cCmp1+"_ITEMPC"),	 NIL})
	EndIf
