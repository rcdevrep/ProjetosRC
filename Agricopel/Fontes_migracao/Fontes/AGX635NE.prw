...
Static Function InserirNFE(cAliasCapa)
	...
	While (cAliasCapa)->(!eof())
		...
		If !(SA2->(DbSeek(xFilial("SA2")+(cAliasCapa)->(CNPJ_CPF))))
			...
			//Inclui fornecedor  
	   	   	U_AGX635CF((cAliasCapa)->(CNPJ_CPF),'SA2')	
        ...
        _cSerie := PADR(StrTran(alltrim((cAliasCapa)->(F1_SERIE)),'-','' ),aTam[1],' ')
		...
		If dbseek(xfilial('SF1')+_cDoc+_cSerie+SA2->A2_COD+SA2->A2_LOJA)  
			...
			If  YEAR(SF1->F1_DTDIGIT) <> Year(ddatabase)
				//GRAVA Array de LOG
	        	AADD(aLogs,{;
					{'ZDB_DBEMP'  ,(cAliasCapa)->(DBEMP)},;
					{'ZDB_DBFIL'  ,(cAliasCapa)->(DBFIL)},;
	 	...
	 	Else
	 		...
	 		If !ValPrefixo( cFil + substr(alltrim((cAliasCapa)->(F1_SERIE)),1,3) , _cDoc , SA2->A2_COD , SA2->A2_LOJA ) 
	 					//GRAVA Array de LOG
	        		AADD(aLogs,{;
						{'ZDB_DBEMP'  ,(cAliasCapa)->(DBEMP)},;
						{'ZDB_DBFIL'  ,(cAliasCapa)->(DBFIL)},;
		...
		RecLock("SF1" , .T.)
			...
			SF1->F1_PREFIXO :=  cFil + substr(alltrim((cAliasCapa)->(F1_SERIE)),1,3)
   ...
   //cQuery +=",COM_CONNFC.STG_FRT_TABCAR_Con_Codigo  AS PLACA_RAT "+CHR(13)// #NOVO
	...
	While (cAliasPROD)->(!eof()) 
		...
		For i := 1 to len(aTemRateio)
			...
			Dbselectarea('CTT')
			DbSetorder(1)
			If !Dbseek(xFilial('CTT')+alltrim(aTemRateio[i][2]))
				lValRat := .F.
			endif
		Next i 
		...
		If 	VAL((cAliasPROD)->D1_CC) == 0 .AND.  VAL((cAliasPROD)->PLACA_CC) == 0 /*.AND. VAL((cAliasPROD)->RAT_CC) == 0 */.AND. 	;
					 !lValRat .AND. (cAliasPROD)->(EX_CC) == 1 
	  			AADD(aLogs,{;
					{'ZDB_DBEMP'  ,(cAliasCapa)->(DBEMP)},;
					{'ZDB_DBFIL'  ,(cAliasCapa)->(DBFIL)},;
...
Static Function ProcCP(xDtDigit)  
	...
	While (cAliasSE2)->(!eof())
		...
		cPrefixo := ""
		cPrefixo := cFilEnt + substr(cSerie,1,3)
...
User Function AGX635JB(xValor,xCte)    
	...
	//Verifico se já existe titulo no contas a pagar
	cQuery := ""
	cQuery := "SELECT R_E_C_N_O_,E2_BAIXA,E2_ORIIMP,E2_EMISSAO,E2_VENCTO FROM " + RetSqlName("SE2") + " "
	...
	If !lJaBaixado  .OR. (lJaBaixado .and. !lValorDif ) 
		...
		conout('AGX635JB - Excluindo dados' ) 
		...
		iF !lJaBaixado //(lJaBaixado .and. !lValorDif )
			...
			cQuery += " WHERE E2_PREFIXO  =  '" +SF1->F1_FILIAL+SUBSTRING(SF1->F1_SERIE,1,3)/*SF1->F1_PREFIXO*/ + "' "
			cQuery += "   AND (E2_NUM     =  '" + SF1->F1_DOC + "') "
			  //	cQuery += "   AND E2_TIPO     =  'NF' "
			cQuery += "   AND E2_FORNECE  =  '" + SF1->F1_FORNECE + "' "
			cQuery += "   AND E2_LOJA     =  '" + SF1->F1_LOJA + "' "
