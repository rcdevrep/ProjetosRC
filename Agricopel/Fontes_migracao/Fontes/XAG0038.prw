...
- Lê informações de pedido dos postos (ZDD e ZDF) e gera pedido de venda do Protheus (SC5/SC6)
...
Static Function CriarPerguntas()
	...
	AADD(aRegistros,{cPerg,"10","Nr Pedido Agricop?","mv_chA","C",6 ,0,0,"G","","mv_par10","","","","","","","","","","","","","","",""})
...
Static Function CarPed()
	...
	If(!Empty(_cStatus))
		_cQuery += " AND ZDC.ZDC_STATUS IN (" + _cStatus + ")"
	EndIf
	...
	If(!Empty(MV_PAR10))
	    ...
	    _cQuery += "            AND   ZDD.D_E_L_E_T_ = '' "
	    _cQuery += "            AND   ZDD.ZDD_NUMSC5 = '" + MV_PAR10 + "')"
	EndIf
	...
	dbUseArea(.T., Nil, _cArqTrab, (_cAliasTrb), .T., .F.)
...
Static Function CriarBrowse()
	...
	AADD(_aCamposBrw, {"Nr Pedido Agricopel" ,"NUMSC5"        , "C", 30, 0, ""})
...
Static Function FiltroStatus()

	Local _cRet := ""

	If (MV_PAR07 != MV_PAR08) .Or. (MV_PAR07 != MV_PAR09) .Or. (MV_PAR08 != MV_PAR09)
		...
		If (MV_PAR08 == 1)
			_cRet += IIf(Empty(_cRet), "'B'", ",'B'")
		EndIf

		If (MV_PAR09 == 1)
			_cRet += IIf(Empty(_cRet), "'C'", ",'C'")
		EndIf
			...
			Begin Transaction
				For _nLoop := 1 To _nLenItens
					...
					If (cC6_LOCAL == "01" .Or. cC6_LOCAL == "03" .Or. cC6_LOCAL == "04")
						Aadd(_aSC5Clone, {"C5_TABELA", "008", Nil})
					Else
...
Static Function isGerarSC5OK(_cAliasPed)
	...
	_cQuery += "             FROM " + RetSqlName("DA1") + " DA1 (NOLOCK) "
	_cQuery += "             WHERE DA1.DA1_CODPRO = ZDD.ZDD_CODPRT "
	_cQuery += "             AND DA1.DA1_CODTAB = '008' "
	_cQuery += "             AND DA1.DA1_FILIAL = '" + xFilial("DA1") + "'"
	_cQuery += "             AND DA1.D_E_L_E_T_ = ''), 0) AS PRECO_008 "
	...
	While !((_cAliasVal)->(Eof()))
		...
		If ((_cAliasVal)->B1_LOCPAD == "01" .Or. (_cAliasVal)->B1_LOCPAD == "03" .Or. (_cAliasVal)->B1_LOCPAD == "04")

			If (Empty((_cAliasVal)->PRECO_008))
	...
	If(!Empty(_cPrecoLub))
		_cMsgErro += Chr(13) + Chr(10) + "- Há produtos dos armazém de Lubs (01/03/04) que não possuem preço cadastrado na tabela 008. Cód Protheus: " + _cPrecoLub
		_lOk := .F.
	EndIf
...
Static Function getSC6(_cAliasPed)
	...
	While !((_cAliasItPed)->(Eof()))
		...
		If ((_cAliasItPed)->B1_LOCPAD == "01" .Or. (_cAliasItPed)->B1_LOCPAD == "03" .Or. (_cAliasItPed)->B1_LOCPAD == "04")
			_nC6PrcVen := (_cAliasItPed)->PRECO_008
		Else
		...
		If (_nC6QtdVen > 0)
			...
			AAdd(_aSC6, {"C6_TES"         , (_cAliasItPed)->F4_CODIGO   , Nil})
			AAdd(_aSC6, {"C6_LOCAL"       , (_cAliasItPed)->B1_LOCPAD   , Nil})
			AAdd(_aSC6, {"C6_CF"       	  , (_cAliasItPed)->F4_CF       , Nil})
			...
			AAdd(_aSC6, {"C6_CLASFIS"     , (_cAliasItPed)->C6_CLASFIS  , Nil})
