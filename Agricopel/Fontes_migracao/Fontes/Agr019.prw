...
PRIVATE nomeprog:="AGR019"
PRIVATE aLinha  := { },nLastKey := 0
PRIVATE cPerg   :="AGR019"
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
titulo+=  OemToAnsi(STR0022)+GetMV("MV_MOEDA"+Str(mv_par05,1))
...
While !Eof() .and. cFilial == E1_FILIAL .and. E1_VENCREA <= mv_par04
	lPVen := .F.
	...
	If E1_PREFIXO < MV_PAR13 .Or. E1_PREFIXO > MV_PAR14
		dbSkip()
		Loop
	EndIf
...
While !Eof()
	...
	nTotPorc += nPorc	
	nTotCount+= nCount
	If (nVendComis <> 0 .or. nTotCount > 0) .or. mv_par08 == 1
...
If mv_par10 > 0  // Aliquota IRRF
	...
	@li, aColu[13] PSAY mv_par10   		Picture "999.99"
