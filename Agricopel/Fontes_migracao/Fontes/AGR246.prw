...
User Function AGR246()
	...
	nLastKey := 0
	lEnd := .f.
	titulo      :="Romaneio de Cargas Rodo Mime"
	cabec1      :=""
	...
	.And. SZB->ZB_NUM    	<= MV_PAR02
		...
		While !Eof() .And. SZB->ZB_NUM == cNum
			...
			.And. SZC->ZC_NUM	 == SZB->ZB_NUM
				...
				@ nLin,023 PSAY cCond
				@ nLin,033 PSAY Transform(SZC->ZC_VALOR,"@E 999,999,999.99")
				@ nLin,050 PSAY Transform(SZC->ZC_VOLUME,"@E 99999")
				@ nLin,060 PSAY Transform(SZC->ZC_PESO,"@E 9999999.99")
				@ nLin,073 PSAY SZC->ZC_CLIENTE
				@ nLin,080 PSAY SZC->ZC_LOJA
				@ nLin,083 PSAY cNome
				...
				If (!Empty(mv_par07) .And. mv_par07 == 1)
					...
					.And. SD2->D2_LOJA    == SZC->ZC_LOJA
						...
						@ nLin,033 PSAY ALLTRIM(SB1->B1_DESC)

						If Len(ALLTRIM(SB1->B1_DESC)) < 50
							@ nLin,034 + Len(ALLTRIM(SB1->B1_DESC)) PSAY Replicate("-",103-(033 + Len(ALLTRIM(SB1->B1_DESC))))
						EndIF
			...
			@ nLin,050 PSAY Transform(nTotVol,"@E 9999")
...
Static Function Cabecalho()
	...
	@ nLin,001 PSAY "NrNota      Serie     Cond.Pgto  Valor R$     Volumes           Peso    Client Lj Nome                    "
	nLin := nLin + 1
	@ nLin,001 PSAY Replicate("-",150)
	nLin := nLin + 1
