...
@param empresa
@type function */    
User function XAG0045(xDatabase,xEmpresas,xGerManual)
  	...
  	For i := 1 to len(_aEmpresas)
		...
		_aConta := U_XAG0045B(cDiaSemana)   
	   	...
	   	If len(_aConta)  > 0 //.AND. alltrim(cMail) <> ''
			...
			mv_par05 := IIF(xGerManual, xDatabase,stod('20190108'))//stod('20180102')//Emissao De  //Data de Inicio da Rotina de Geração      
			...
			cQuery := ""
			cQuery := " SELECT F2_DOC AS DOC, F2_SERIE AS SERIE, F2_CLIENTE AS CLIENTE, F2_LOJA AS LOJA, F2_EMISSAO AS EMISSAO, A1_NOME AS NOMECLI,A1_CGC AS CNPJ "
			...
			cQuery += " A1_FILIAL = '"+xFilial('SA1')+"' AND A1_XBOLETO = '2' "
			...
			//Fim              
			cQuery += " AND NOT (E1_SITUACA = '0' AND E1_NUMBCO <> '') "
			cQuery += "GROUP BY F2_FILIAL, F2_DOC, F2_SERIE, F2_CLIENTE, F2_LOJA, F2_EMISSAO, A1_NOME, A1_CGC "
			...
			TCSetField("TRB","E1_DATABOR","D",08,0) 
