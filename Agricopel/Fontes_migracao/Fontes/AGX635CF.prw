...
User Function AGX635CF(xCNPJ,xTipo)
	...
	cArqTmp := CriarArqCF(xCNPJ)

	cAliasCF := GetNextAlias()
	DbUseArea(.T., Nil, cArqTmp, (cAliasCF))
	...
	While (!(cAliasCli)->(Eof()))
		...
		If (CGC((cAliasCli)->(CNPJ_CPF))) // Validação de CPF/CNPJ
			...
			lNovoCli  := !SA1->(DbSeek(xFilial("SA1")+(cAliasCli)->(CNPJ_CPF)))
     		...
     		If cCodNovo == '01'
     	  		_cConta := U_X635CONT(NoAcento(AnsiToOem((cAliasCli)->(FANTASIA))),"SA1")  
     		Else
     		...
     		If ('AGX635' $ SA1->A1_OBSERV .and. !lNovoCli) .or. lNovoCli
				...
				SA1->A1_COD_MUN := (cAliasCli)->(IBGE)
				SA1->A1_MUN_ANP := (cAliasCli)->(IBGE)
				SA1->A1_MUN     := (cAliasCli)->(MUNIC)
				SA1->A1_CEP     := (cAliasCli)->(CEP)
...
User Function x635NEXT(xTipo)
 	...
 	iF xTipo = 'SA1'
		...
		cQuery += " WHERE CT1.D_E_L_E_T_ = ''   "
   		cQuery += " AND   CT1.CT1_CONTA BETWEEN '112010000' AND '112019999' "
		cQuery += " AND   LEN(RTRIM(CT1.CT1_CONTA)) = 9  "
		...
		cQuery += " 		  AND   CT1SUB.CT1_CONTA BETWEEN '112010000' AND '112019999' "
		cQuery += " 		  AND   LEN(RTRIM(CT1.CT1_CONTA)) = 9 ) "			  
 	Else
		/*cQuery += " SELECT MAX(CT1_CONTA) AS CT1_CONTA FROM CT1"+cEmpant+"0"
