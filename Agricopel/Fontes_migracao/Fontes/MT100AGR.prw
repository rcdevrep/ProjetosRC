...
User Function MT100AGR()
	...
	cQuery := ""
	cQuery += " SELECT F1.F1_DOC, R_E_C_N_O_, F1_CHVNFE, F1_ESPECIE "
	cQuery += " FROM " + RetSqlName("SF1") + " F1 (NOLOCK) "
	cQuery += " WHERE LTRIM(RTRIM(F1.F1_FILIAL))  = '" + xFilial("SF1") + "' "
	cQuery += "   AND LTRIM(RTRIM(F1.F1_DOC))     = '" + Trim(SF1->F1_DOC)  + "' "
	cQuery += "   AND LTRIM(RTRIM(F1.F1_SERIE))   = '" + Trim(SF1->F1_SERIE) + "' "
	cQuery += "   AND LTRIM(RTRIM(F1.F1_FORNECE)) = '" + Trim(SF1->F1_FORNECE) + "' "
	cQuery += "   AND LTRIM(RTRIM(F1.F1_LOJA))    = '" + Trim(SF1->F1_LOJA) + "' "
	...
	TCQuery cQuery NEW ALIAS "GL1"

    dbSelectArea("GL1")
	dbgotop()
  	While !Eof()
		If Trim(GL1->F1_ESPECIE) == 'SPED' .AND. Trim(GL1->F1_CHVNFE) == ""
			U_AGX321()
		EndIf
		GL1->(dbskip())
	EndDo
