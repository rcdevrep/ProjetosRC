...
Static Function Copiar001()
	...
	If (MV_PAR01 == '700') .Or. (AllTrim(MV_PAR01) $ "002;003;004;005;007" .And. cEmpAnt == "01" .and. cFilAnt == "06")
		...
		While !Eof()
			...
			DbSetOrder(3)
			DbGotop()
			DbSeek(xFilial("DA1")+MV_PAR01+(cAliasDA1)->DA1_ITEM,.T.)
			While !Eof() .And. DA1->DA1_FILIAL == xFilial("DA1") .And.;
			DA1->DA1_CODTAB == MV_PAR01         .And.;
			DA1->DA1_ITEM   == (cAliasDA1)->DA1_ITEM
				If ALLTRIM(DA1->DA1_CODPRO) == ALLTRIM((cAliasDA1)->DA1_CODPRO)
					cProTab := 'S'
				Else
					RecLock("DA1",.F.)
					DbDelete()
			...
			If Empty(cProTab)
				...
				IncProc("Produto incluido..."+(cAliasDA1)->DA1_CODTAB+" "+(cAliasDA1)->DA1_ITEM+" "+(cAliasDA1)->DA1_CODPRO)
...
Static Function AGR252REL()
	...
	RptStatus({|lEnd| AGR252IMP(@lEnd,wnrel,cString)},Titulo)
Return
