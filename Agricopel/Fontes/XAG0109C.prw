...
/*/
User FUNCTION XAG0109C(_cPedido, _cSeqPed) //U_XAG0109C('A13930')

	Default _cSeqPed := ""
	Local _aArea     := {}
	Local _aImpress  := GetImpWindows(.F.) // Busca impressoras cadastradas na maquia
	...
	//Atribui a impressora padrao da maquina
	iF len(_aImpress) > 0 
		If alltrim(_aImpress[1]) <> ''
			_cPrinter := _aImpress[1]
		Endif 
	Endif 
	...
	If _lRet
		Processa( {|_lRet| u_109IDANFE(_cDoc, _cSerie) }, "Aguarde...", "Realizando Impress„o da Nota Fiscal...",.T.)//U_109IDANFE('000012706','2')
	Endif
...
User Function 109CNota( _cPedido,_cSeqPed)
	...
	Else
		MsgAlert('Pedido '+_cPedido+' nao encontra-se Liberado, verifique!', 'ATENCAO')
		_lRet := .f.
	Endif
...
User Function 109GDANFE()
	...
	_cRetMnt := U_CTNFeMnt(_cSerie, _cDoc, _cDoc) //-> monitora a nfe
	If valtype(_cRetMnt) <> 'C'
		U_109CTAutoNfe(cEmpAnt,xFilial("SF2"),"0",cANfeAmbi,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_DOC)
		_cRetMnt := ''
	Endif 
	...
	While !(UPPER('DANFE autorizada') $ UPPER(_cRetMnt) ) .and. !lCanceled 
		
		Sleep(_nSlpANFe)
	
		_cRetMnt := U_CTNFeMnt(_cSerie, _cDoc, _cDoc) 
		If valtype(_cRetMnt) <> 'C'
			_cRetMnt := ''
			U_109CTAutoNfe(cEmpAnt,xFilial("SF2"),"0",cANfeAmbi,SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_DOC)
		Endif 

		If !(UPPER('DANFE autorizada') $ UPPER(_cRetMnt) ) 
			If !(msgYesno('XAG0109C - Transmissao com erro: '+alltrim(_cRetMnt)+', deseja tentar novamente?'))
				lCanceled := .T.
			Endif
		Endif 
	...
	If !(UPPER('DANFE autorizada') $ UPPER(_cRetMnt) )
		MsgAlert('XAG0109C - Entre em contato com o faturista, pois a nota '+_cDoc+' n„o foi transmitida corretamente!','ATENCAO')
	Endif 
...
//  Acha nota fiscal
	...
	if _cBoleto == "S"   
		If SA1->A1_XBOLETO <> '3'// Devera imprimir boleto		
		...
		Else
			MsgAlert("XAG0109C: Cliente cadastrado para n„o geraÁ„o de Boleto!","Aviso")
		Endif	
	else
