...
User Function 635ZIMP()
	...
	Local cHora     := ""    
	Local cMinutos  := ""
	Local _cAmbiente := alltrim(GetEnvServer())
	
	//Bloqueio de Rotina enquanto Executando Schedule que roda de Hora em Hora
	If !("HOM" $ Upper(_cAmbiente) .Or. "MIG" $ Upper(_cAmbiente))   //!(substr(alltrim(GetEnvServer()),1,10) == 'N2SD9W_HOM'	.OR. substr(alltrim(GetEnvServer()),1,12) == 'N2SD9W_MIGRA' ) 
		cHora 	 := SUBSTR(TIME(), 1, 3)  
		cMinutos := SUBSTR(TIME(), 4, 2)
		If (Val(cMinutos) < 10 .or. Val(cMinutos) > 55) .and.!(FWIsAdmin(__cUserId))// __cUserID <> '000000'//Só libera a Utilização após 10 minutos e antes de 5 Minutos da execução do Schedule
...
User Function 635ZDT6()
	...
	If ! ((__cUserID $ GetMV('MV_XIMPDT6') )  .OR. FWIsAdmin(__cuserid) ) 
	...
	AADD(aParamDT6,DateMySql(MV_PAR05,'')) 
	AADD(aParamDT6,DateMySql(MV_PAR06,''))
...
User Function 635ZUPD()
	...
	Local lCteEnt    := iif(alltrim(ZDB->ZDB_ROTINA) == 'AGX635CE',.T.,.F. )		 
	Local cQuery     := ""
	Local lErroSQL   := .F. 
	Local lExistUPD  := .F.
	Local i          := 0
 	...
 	Do Case 
		...
		Case  lCteEnt   
	   		...
	   		If len(_aCampos) > 1 .and. !(Empty(ZDB->ZDB_DBEMP)) .and. !(Empty(ZDB->ZDB_DBFIL))
				...
				cQuery += " COM_CONFRE_DHIntTotvs = '2000-01-01 00:00:00' "
				cQuery += " WHERE COM_CONFRE_DHIntTotvs IS NULL "
				cQuery += " AND STG_GEN_TABEMP_Codigo 	  =  "+ZDB->ZDB_DBEMP+""
				cQuery += " AND STG_GEN_TABFIL_Codigo =  "+ZDB->ZDB_DBFIL+""
		   		cQuery += " AND COM_CONFRE_Numero 		  =  "+_aCampos[1]+""
				cQuery += " AND COM_CONFRE_Serie 		  =  '"+_aCampos[2]+"' "
				cQuery += " AND STG_COM_CONFRE_Tra_Codigo =  "+_aCampos[3]+""
				If len(_aCampos) > 3 
					cQuery += " AND STG_COM_CONFRE_Etr_Codigo =  "+_aCampos[4]+""
		   		Endif 
		   		U_AGX635CN("DBG") 
