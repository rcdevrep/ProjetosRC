					...
					conout(' AGX635D6 '+cEmpPara+' / '+cFilialPara)
				  	PREPARE ENVIRONMENT Empresa cEmpPara Filial cFilialPara Tables "SA1","SA2","SB1","SF2","SD2","SF3","SE1","SF4","SX5","DT6","DTC","XXS"
				  		...
				  		If RPCSetEnv(cEmpPara, cFilialPara)
							...
							/*oTmpTable := CriaArqD6(nEmpDe,nFilde,aParamDT6)
							cCapaCTS := oTmpTable:GetAlias()*/
							cCapaCTS := CriaArqD6(nEmpDe,nFilde,aParamDT6)
							If Select(cCapaCTS) <> 0
								...
								dbCloseAll()
							Else
								conout("AGX635D6 - Nao foi Possivel selecionar : "+cCapaCTS)
							Endif 
						Else
							conout("AGX635D6 - Nao foi Possivel abrir o ambiente: "+cEmpPara+"-"+cFilialPara)
						Endif 
...
Static Function SelectCTS(nEmpOrigem,nFilOrigem,aParamDT6)
    ...
    Local cCapaCTS := "SelectCTSA"//GetNextAlias()
    Local cQuery    := "" 
    Local cDatade   := ""
	Local cDataAte  := ""
	...
	If Len(aParamDT6) > 5
		...
		cQuery += " AND CTE_MOVCTE_Serie   <=  '"+aParamDT6[2]+"' "  //Serie Ate
		 ...
		 //Se for schedulado roda a partir do ultimo fechamento
		If aParamDT6[9]
			cQuery += " AND CTE_MOVCTE_Emissao >=  '"+DateMySql( GETMV(MV_ULMES) + 1,'')+"' "  //Emissao de 
	...
	Else                                                                                
		...
		cQuery += " AND CTE_MOVCTE_Emissao >=  '"+DateMySql(stod(cDatade),'')+"' "  //Emissao de 
		cQuery += " AND CTE_MOVCTE_Emissao <=  '"+DateMySql(stod(cDataAte),'')+"' "  //Emissao Ate	
	Endif	
...
Static Function INSERIRDT6(cCapaCTS)
	...
	//	Endif    		
		...
		//Grava tipo Tms conforme Tipo da Nota 
		if alltrim(SF2->F2_TIPO) == 'N'
	    	cTipoTMS := "2"     
		ElseIf alltrim(SF2->F2_TIPO) == 'C'
			cTipoTMS := "8"   
		Elseif alltrim(SF2->F2_TIPO) == 'I'
			cTipoTMS := "M"   
		Elseif alltrim(SF2->F2_TIPO) == 'D'
			cTipoTMS := "6"
		Else
			cTipoTMS := ""   
		Endif 
...
Static Function CriaArqD6(nEmpOrigem,nFilOrigem,aParamDT6)
   ...
   //RPCSetEnv("01", "01")
	...
	If Select(cAliasQry) <> 0
		...
		cAliasArea := "CriaArqD6"//GetNextAlias()
		cArquivo := CriaTrab(,.F.)
...
User Function AGX635DS()
	...
	//dDataINI :=  //dtos( GETMV(MV_ULMES) + 1)//dtos(date())
	//cDataINI := substr(cDataINI,1,6)+'01'      
	//conout('AGX635DS - Data inicial: '+dtos(dDataINI))
	AADD(aParamDT6,'   ')   //Serie de 
	AADD(aParamDT6,'ZZZ') //Serie ate 
	AADD(aParamDT6,'         ')//Cte de  
	AADD(aParamDT6,'999999999')//Cte ate 
	AADD(aParamDT6,DateMySql(date(),'')) //Emissao de 
	AADD(aParamDT6,DateMySql(date(),'')) //Emissao ate
	AADD(aParamDT6,'01')   
	AADD(aParamDT6,'99')  
	AADD(aParamDT6,lScheddt6)   
	//AADD(aParamDT6,'01')  
    ...
    If xOPC == 1     
 	...
 	//Grava tabela DTC	  			   	
   	ElseIf xOPC == 2
   		dbselectarea('DTC')
   		dbsetorder(1)
		conout(xFilial('DTC') + DT6->DT6_FILORI + DT6->DT6_LOTNFC + '|')
   		If DTC->( DbSeek(xFilial('DTC') + DT6->DT6_FILORI + DT6->DT6_LOTNFC ) )//dbseek(DT6->DT6_FILIAL+DT6->DT6_DOC+DT6->DT6_SERIE+DT6->DT6_FILDOC)//+DTC_NUMNFC+DTC_SERNFC ////Dbseek(xFilial('DTC') + DT6->DT6_FILDOC + DT6->DT6_LOTNFC )
   			lNovoDTC := .F.	
   		Else
   			lNovoDTC := .T.	
   		Endif 
