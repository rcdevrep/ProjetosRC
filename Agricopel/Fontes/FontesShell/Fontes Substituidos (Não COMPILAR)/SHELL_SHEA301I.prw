	...
	If DbSeek(xFilial("SA1")+cCodCli)
	   ...
	   //	cQryVerif += 	"AND C6_NUM	   in	("+cProd+") "
		...
		If !VERIF->(Eof()) .Or. !VERIF2->(Eof())
			...
			If !lBat
				MSGInfo("Jแ existe no Protheus o pedido " + If(!VERIF->(Eof()),"1: "+VERIF->C5_NUM,"2: "+VERIF2->C5_NUM) + " com as mesmas caracteristicas - O Pedido do Palm " + cNumPed + " / CGC " + cCGCDist + " nใo serแ importado", "Importa็ใo de pedidos")
			Else
				ConOut("Jแ existe no Protheus o pedido " + If(!VERIF->(Eof()),"1: "+VERIF->C5_NUM,"2: "+VERIF2->C5_NUM) + " com as mesmas caracteristicas - Pedido do Palm " + cNumPed + " / CGC " + cCGCDist + " nใo serแ importado")
				EditTxt("\system\SHEA301I.log","Jแ existe no Protheus o pedido " + If(!VERIF->(Eof()),"1: "+VERIF->C5_NUM,"2: "+VERIF2->C5_NUM) + " com as mesmas caracteristicas - Pedido do Palm " + cNumPed + " / CGC " + cCGCDist + " nใo serแ importado")
			Endif
		...
		While (cAliasIten)->( !Eof() )
			...
			If MsSeek(xFilial("SB1")+(cAliasIten)->CODPROD)
				...
				If MsSeek(xFilial("SF4")+SB1->B1_TS)
					...
					cCFO   := "" //SF4->F4_CF Comentado em 09/06/2020 ap๓s Thiago (da Agricopel) ter relatado problema na importa็ใo de Pedidos inserido em outro Estado
					cTES   := SF4->F4_CODIGO
					cLocal := ""
			...
			If FindFunction("U_CHKCOMBO").AND. U_CHKCOMBO() .AND. DbSeek(xFilial("SG1") + (cAliasIten)->CODPROD)
				...
				n := 1
				aCols := {}
				/*COMPATIB_DIC - Comentado por Max Ivan (Nexus) em 10/03/2020 - Substituํdo pelo c๓digo abaixo
				SX3->(DbSetOrder(1))
				SX3->(MsSeek('SC6'))
			...
			Else
			   ...
			   EndIf
				...
				// Calcula o pre็o de Venda para Querosene = (Pre็o tabela - Valor de ST) 
				If SC6->(FieldPos("C6_XVLTST")) > 0 .and. SB1->B1_TIPO == 'QR'
		...
		If !Empty(aCab) .And. !Empty(aItens)
			...
			If	!lMsErroAuto .OR. lBloq
				...
				If lBloq
					If !lBat
						//MSGInfo("Pedido importado mas com bloqueio ou alguma ocorr๊ncia", "Importa็ใo de pedidos") Retirado e colocado somente uma mensagem em 04/10/2018 por Max Ivan
				...
				Else
					If !lBat
						//MSGInfo("Pedido importado com sucesso", "Importa็ใo de pedidos") Retirado e colocado somente uma mensagem em 04/10/2018 por Max Ivan
...
//Mensagem ๚nica de importa็ใo de PVs montada em 04/10/2018 por Max Ivan (Nexus)
If !lBat
   If !Empty(_cPedsBlq)
      MSGInfo("Pedidos importados com bloqueio ou alguma ocorr๊ncia: "+SubsTr(_cPedsBlq,3), "Importa็ใo de pedidos")
   EndIf
   If !Empty(_cPedsOK)
      MSGInfo("Pedidos importados com sucesso: "+SubsTr(_cPedsOK,3), "Importa็ใo de pedidos")
   EndIf
EndIf
...
While !(cAliasVis)->(EOF())
	EditTxt("\system\SHEA301I.log",'Iniciando atualiza็ใo do registro de VISITA, para o Vendedor: '+(cAliasVis)->CODVEND+' - Hora Inicio: '+(cAliasVis)->HORARIO_INICIO)
	...
	If AD7->( DbSeek(cSeek) ) .and. !Empty(dDtRoteiro) //Feito por MAX IVAN em 03/11/2014 - pedido por Flแvio Mansur, para atender caso de atualizar o potencial do cliente sem atualizar o PAN e AD7
        ...
        If _cInsVis == "S" .and. !Empty((cAliasVis)->OBS1) .and. (AllTrim(AD7->AD7_HORA1) # "08:00" .or. AllTrim(AD7->AD7_HORA2) # "08:00") .and. Empty(dDtRemarc)
           If RecLock("AD7",.T.)
              ...
              MsUnLock()
              _lUpdVis   := .T.
              EditTxt("\system\SHEA301I.log",'Inserida a visita na condi็ใo 1')
           EndIf
        Else
           RecLock("AD7",.F.)
		   AD7->AD7_STATUS := (cAliasVis)->STATUVIS
		   ...
		   AD7->AD7_PROXPA := (cAliasVis)->PROXPA   //Pr๓ximos Passos
		   _lUpdVis   := .T.
           EditTxt("\system\SHEA301I.log",'Alterada a visita na condi็ใo 1')
        EndIf
		...
		If !Empty(dDtRemarc)
			...
			If !Empty(dDtRoteiro)//alterado para considerar apenas registros com data
				If AD7->(!Found() )
					...
					MsUnLock()
					_lUpdVis   := .T.
                    EditTxt("\system\SHEA301I.log",'Inserida a visita na condi็ใo 2')
				Else
					RecLock("AD7",.F.)
					AD7->AD7_FILIAL      := xFilial("AD7")
					AD7->AD7_TOPICO      := cTopico
					...
					MsUnLock()
					_lUpdVis   := .T.
                    EditTxt("\system\SHEA301I.log",'Alterada a visita na condi็ใo 2')
				EndIf
			EndIf
		Else
			MsUnlock()
	...
	If lRet
	   ...
	   //	SA1->A1_TEL		:= Iif (!Empty((cAliasVis)->TEL1),(cAliasVis)->TEL1,SA1->A1_TEL)
		...
		//REGRAVAวรO DE CAMPOS DE CLIENTES - Ajuste feito em 03/04/2019, เ pedido da Lubpar, e autorizado pelo Flแvio
...
If !Empty(_cVisitas)
   MSGInfo("Importado com sucesso visitas para os vendedores: "+SubsTr(_cVisitas,3), "Importa็ใo de visitas")
EndIf
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณSHEA301NewCliบAutor  ณMicrosiga           บ Data ณ  06/10/11บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
...
While !(cAlias)->(EOF())
	...
	If (cAlias)->PROSPECT == "1"
		...
		EndIf
		If !lBat
			MSGInfo("Os dados do Prospect "+SUS->US_COD+" foram importados com sucesso", "Importa็ใo de prospect")
		Else
		...
		EndIf
		EditTxt("\system\SHEA301I.log",'Incluํdo Prospect: '+SUS->US_COD+' - '+(cAlias)->RAZAO)
	...
	ElseIf (cAlias)->PROSPECT == "2"
		...
		If __lSX8
			ConfirmSX8()
		EndIf
		EditTxt("\system\SHEA301I.log",'Incluํdo Prospect: '+SA1->A1_COD+' - '+(cAlias)->RAZAO)
		cDSemana := CVALTOCHAR((cAlias)->DSEMANA)  //(cAlias)->DSEMANA
		
		If !lBat
			MSGInfo("Os dados do Cliente "+SA1->A1_COD+"/"+SA1->A1_LOJA+" foram importados com sucesso", "Importa็ใo de cliente")
		Else
	...
	If TcSqlExec(cQuery) < 0
	   Alert("Erro no UPDATE de atualiza็ใo da data de leitura do Cliente: "+TCSQLError())
		//RestArea(aArea)
		If InTransact()
			DisarmTransaction()
		EndIf
