...
User Function NX2460i()      
	...
	ElseIf (cEmpAnt == "01" .and. SF2->F2_filial == "02") .or. cEmpAnt == "16"
		SCalcPien()
	ElseIf (cEmpAnt == "01" .and. (SF2->F2_filial == "03" .or. SF2->F2_filial == "15" .or. SF2->F2_filial == "16" )) .or. (cEmpAnt == "11" .and. SF2->F2_filial == "01") .or. (cEmpAnt == "12" .and. SF2->F2_filial == "01") .or. (cEmpAnt == "15" .and. SF2->F2_filial == "01")
		SCalcBase()
	ElseIf cEmpAnt == "02"
		SCalcMime()
	EndIf
	...
	If SF2->F2_TIPO == "N" .OR. SF2->F2_TIPO == "B"
		nPesLiq 	:= 0
		nPesBru 	:= 0
		nVol1   	:= 0
		cEsp1		:= ""
		...
		//Quando for Exportação deve pegar peso do Pedido de Venda
		If alltrim(SC5->C5_TIPOCLI) == 'X' .and. !Empty(SC5->C5_PESOL) .and. !Empty(SC5->C5_PBRUTO)
			nPesLiq := SC5->C5_PESOL
			nPesBru := SC5->C5_PBRUTO
		Endif 
...
Else
	PerMTC010()
    nVlrCpr := MaPrcPlan(SD2->D2_COD,"SLA_AGRS","CUSTO_TOTAL_DA_COMPRA",0)  //CUSTO TOTAL DA COMPRA (j)	
	nPerVds := MaPrcPlan(SD2->D2_COD,"SLA_AGRS","PERC_REF_VENDAS",0)        //PERCENTUAL DE REFERENCIA PARA CALCULO DO CUSTO DA VENDA  (j)
 	nPerMgr := MaPrcPlan(SD2->D2_COD,"SLA_AGRS","PERC_REF_MARGEM",0)  //PORCENTUAL MARGEM CONTRIBUICAO (j)  
Endif   
...
If DbSeek(xFilial("SF4")+SD2->D2_TES)
	...
	If SF4->F4_ICM = "S"
		...
		If SD2->D2_TES == "513" .AND.  SM0->M0_CODIGO = "01" .AND. Alltrim(SM0->M0_CODFIL) = "02"
			nPerRet := 12
		EndIf
