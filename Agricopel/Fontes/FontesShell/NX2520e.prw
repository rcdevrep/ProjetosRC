...
//			EstApont()
			//Caso não tenha Apontamento ignora
			If alltrim(SC6->C6_NUMOP) <> ''
...
Static Function ExcApont()
	...
	If SM0->M0_CODIGO == '03' .OR. (SM0->M0_CODIGO == '01' .AND. Alltrim(SM0->M0_CODFIL) == '03') // Mime Distrib. ou Agricopel Base
	   aAdd(aVetor, {"D3_TM"     , "010"				, Nil})
	Endif
	If SM0->M0_CODIGO == '44' // Posto Farol
	   aAdd(aVetor, {"D3_TM"     , "002"				, Nil})
	Endif	
	If (SM0->M0_CODIGO == '01' .AND. (Alltrim(SM0->M0_CODFIL) == '01' .OR. Alltrim(SM0->M0_CODFIL) =="06")) .OR. (SM0->M0_CODIGO == '01' .AND. Alltrim(SM0->M0_CODFIL) == '02') // Agricopel Matriz ou Pien
