...
Static Function CriaBrw(lMostraFil)
	...
	oBrowseEmp:AddButton("Confirmar", { || Confirmar()},,,, .F., 2 )

	oBrowseEmp:bAllMark := { || CheckAll(oBrowseEmp:Mark() ,lMarcar := !lMarcar), oBrowseEmp:Refresh(.T.)}

	If (lMostraFil)
    	oBrowseEmp:SetColumns(MontaColunas("M0CODIGO",  "Código Empresa", 1, "", 1,  2, 0))
		oBrowseEmp:SetColumns(MontaColunas("M0CODFIL",  "Código Filial", 1, "", 1,  2, 0))
		oBrowseEmp:SetColumns(MontaColunas("M0NOMECOM", "Nome Empresa"  , 1, "", 1, 60, 0))
		oBrowseEmp:SetColumns(MontaColunas("M0NOME", "Fantasia"  , 1, "", 1, 60, 0))
		oBrowseEmp:SetColumns(MontaColunas("M0FILIAL", "Nome Filial"  , 1, "", 1, 60, 0))
	Else
    	oBrowseEmp:SetColumns(MontaColunas("M0CODIGO",  "Código Empresa", 1, "", 1,  2, 0))
		oBrowseEmp:SetColumns(MontaColunas("M0NOMECOM", "Nome Empresa"  , 1, "", 1, 60, 0))
	EndIf
...
Static Function CriaTRBEmp()
	...
	Aadd(aCampos,{ "M0CODIGO", "C", 2, 0 } )
	Aadd(aCampos,{ "M0NOMECOM", "C", 60, 0 } )

	oTrb := FWTemporaryTable():New()
	oTrb:SetFields(aCampos)

	oTrb:AddIndex("IDX1", {"M0CODIGO"})
	...
	While !SM0->(Eof())
		...
		If (cUltCod <> SM0->M0_CODIGO)
			...
			(cAliasArea)->M0CODIGO := SM0->M0_CODIGO
			(cAliasArea)->M0NOMECOM := SM0->M0_NOMECOM
...
Static Function CriaTRBFil()
    ...
    Aadd(aCampos,{ "TAB_OK", "C", TamSx3("C5_OK")[1], 0 } )
	Aadd(aCampos,{ "M0CODIGO", "C", 2, 0 } )
	Aadd(aCampos,{ "M0CODFIL", "C", 2, 0 } )
	Aadd(aCampos,{ "M0NOMECOM", "C", 60, 0 } )
	Aadd(aCampos,{ "M0NOME", "C", 60, 0 } )
	Aadd(aCampos,{ "M0FILIAL", "C", 60, 0 } )

	oTrb := FWTemporaryTable():New()
	oTrb:SetFields(aCampos)

	oTrb:AddIndex("IDX1", {"M0CODIGO", "M0CODFIL"})
	...
	While !SM0->(Eof())
		...
		(cAliasArea)->M0CODIGO := SM0->M0_CODIGO
		(cAliasArea)->M0CODFIL := Alltrim(SM0->M0_CODFIL)
		(cAliasArea)->M0NOMECOM := SM0->M0_NOMECOM
		(cAliasArea)->M0NOME := SM0->M0_NOME
		(cAliasArea)->M0FILIAL := SM0->M0_FILIAL
