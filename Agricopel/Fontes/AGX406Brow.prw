...
User Function AGX460B(pcFiltro)
	...
	MarkBrowse(cAliasTRB,"OK",cAliasTRB+"->DB_IMPMAP",aCamposArq,, cMarca)
...
Static Function CriarArqTrab()
	...
	oTmpTable:AddIndex("1", {"A1_COD","DB_DOC"})
	oTmpTable:Create()
...
Static Function Car_Pedido()
	...
	cQuery += "  FROM " + RetSqlName("SDB") + " SDB (NOLOCK), " + RetSqlName("SC5") + " SC5 (NOLOCK), " + RetSqlName("SA1") + " SA1 (NOLOCK) "
...
Static Function Car_Carga()
	...
	cQuery += "  FROM " + RetSqlName("SDB") + " SDB (NOLOCK) "
...
//			cPedidos += If(mv_par11 == 1, " DB_DOC == '", " DB_CARGA == '") + (cAliasTRB)->DB_DOC + "'"
			if cPedidos <> ""
				cPedidos += ","
			Endif

			cPedidos += "'" + ALLTRIM((cAliasTRB)->DB_DOC) + "'"
