...
User function AGX500(xAliAGX498)
	...
	Private aRegistros := {}
	Private cNrConf    := ""
	Private cAliAGX498 := xAliAGX498

	cNrConf := (cAliAGX498)->NUM_CONF
...
Static Function ValidarConf()
	...
	cQuery += " FROM SF1010 (NOLOCK), ZZK010 (NOLOCK) "
...
Static Function MontarItens()
	...
	For nX := 1 to Len(aFields)
		...
		If aFields[nX] = "Lote"
			Aadd(aHeaderEx, {"Lote","USA_LOTE","",3,0,,GetSX3Cache("ZZJ_PRODUT", "X3_USADO"),"C",,,,})
		ElseIf aFields[nX] = "LOCAL_NF"
			Aadd(aHeaderEx, {"Armazém NF","LOCAL_NF","",3,0,,GetSX3Cache("ZZJ_PRODUT", "X3_USADO"),"C",,,,})
		Else
...
Static Function oGravar()
	...
	If ValidarGravacao()
		...
		DbSelectArea(cAliAGX498)
		RecLock(cAliAGX498,.F.)
		(cAliAGX498)->STATUSCONF := IIF(lConfOK, "B","I")
		MsUnLock()
		...
		If (lConfOK)
			...
			If (ZZI->(FieldPos("ZZI_DTBAIX")) > 0)
				ZZI->ZZI_DTBAIX := Date()
				ZZI->ZZI_HRBAIX := Substr(Time(),1,5)
				ZZI->ZZI_USRBAI := UsrRetName(RetCodUsr())
			EndIf
		Else
			ZZI->ZZI_STATUS := "I"
		EndIf
...
Static Function CarEmail()
	...
	cQuery += " FROM SB1010 SB1 (NOLOCK), SD1010 SD1 (NOLOCK), ZZK010 ZZK (NOLOCK) "
