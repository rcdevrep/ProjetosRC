...
//	cRet := "((E5_TIPODOC <> 'DH' .AND. E5_TIPODOC <> 'CA' .AND. E5_TIPODOC <> 'CH' .AND. E5_ARQCNAB <> '') .OR. (E5_TIPODOC <> 'DH' .AND. E5_TIPODOC <> 'CH' .AND. E5_ARQCNAB == '') .OR. (E5_BANCO <> '237' .AND. SUBSTR(E5_HISTOR,1,5) == 'TALAO' .AND. E5_ARQCNAB == '') .OR. E5_MOEDA == 'M1')"
    ...
    If !("CUSTOM" $ Upper(_cAmb)) 
        cRet := "((E5_TIPODOC <> 'DH' AND E5_TIPODOC <> 'CA' AND E5_TIPODOC <> 'CH'" +;
        " AND E5_ARQCNAB <> '' ) OR (E5_TIPODOC <> 'DH' AND E5_TIPODOC <> 'CA' AND E5_TIPODOC <> 'CH' " +;
        " AND E5_ARQCNAB = '' ) OR (E5_BANCO <> '237' AND SUBSTRING(E5_HISTOR,1,5) = 'TALAO' AND E5_ARQCNAB = '' ) " +; 
        " OR E5_MOEDA =  'M1') "
    Endif
Return cRet
