		...
		if lPrimeira
			...
			For nX := 1 To Len(aAux)
				If Upper(AllTrim(aAux[nX]:_TPMED:Text)) == "PESO BRUTO"
					...
					If (nPesoBruto >= 9999999)

						If lJob
							aAdd(aErros,{cFile, "CT-e possui valor de peso acima do máximo permitido (" + Str(nPesoBruto) + ")","Solicite ao fornecedor para reenviar um CT-e corrigido."})
						Else
							Alert("CT-e possui informação de peso bruto incorreta (" + Str(nPesoBruto) + ")","Solicite ao fornecedor para reenviar um CT-e corrigido.")
						EndIf
				...
				If Upper(AllTrim(aAux[nX]:_TPMED:Text)) == "PESO LIQUIDO"
					...
					If (nPesoLiqui >= 9999999)

						If lJob
							aAdd(aErros,{cFile, "CT-e possui valor de peso acima do máximo permitido (" + Str(nPesoLiqui) + ")","Solicite ao fornecedor para reenviar um CT-e corrigido."})
						Else
							Alert("CT-e possui informação de peso bruto incorreta (" + Str(nPesoLiqui) + ")","Solicite ao fornecedor para reenviar um CT-e corrigido.")
						EndIf
