...
User Function AGX525()
	...
	If (Empty(SF1->F1_FORMUL)) .And. (SM0->M0_CODIGO == "01") .And. (Alltrim(SM0->M0_CODFIL) == "06") .And. AllTrim(SF1->F1_ESPECIE) <> "CTE"
		...
		cQuery += " FROM " + RetSQLName("SD1") + " AS SD1 (NOLOCK), " + RetSQLNAme("SB1") + " AS SB1 (NOLOCK), " + RetSQLNAme("SF4") + " AS SF4 (NOLOCK) "
		...
		cQuery += " AND	  D1_FILIAL	 = SF4.F4_FILIAL "
...
Static Function EnviarEmail()
	...
	oHtml:ValByName("numero", SF1->F1_DOC + '-' + SF1->F1_SERIE )
	...
	Else
		If AllTrim((cAlias)->B1_LOCPAD) == "01" .And. AllTrim((cAlias)->F4_ESTOQUE) == "S"
	...
	While (cAlias)->(!Eof())
		...
		aAdd( (oHtml:ValByName( "produto.armazem" )), (cAlias)->D1_LOCAL )
