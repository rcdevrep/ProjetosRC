...
LOCAL nPreco  := 0, nDesc   := 0, nMoeda  := 1, nQtde   := 0
...
If Len(_aVlrUnit) < N
   For _nI := 1 To N
      If Len(_aVlrUnit) < _nI
	     AAdd(_aVlrUnit,0)
	  EndIf
   Next
EndIf

//Alert(Alltrim(ReadVar()))
...
If IsInCallStack("TMKA271") //Chamada pela Rotina de Televendas
	...
	nCodPai   := aScan(aHeader,{|x| alltrim(x[2])=="UB_CODPAI"})
	
   If cEmpAnt == "01"
      //MsgInfo("Linha: "+StrZero(N,3))
	  //Alert(_nVlrUnit)
      aCols[N][nPosTab]   := aCols[N,nPAuxTab] + ROUND(((aCols[N,nPAuxTab]* M->UA_X_ACRES) / 100),4)
   EndIf

	If Alltrim(ReadVar())=="M->UB_VRUNIT" .and. Empty(aCols[N][nCodPai])
	   nPreco    := M->UB_VRUNIT
	   //_nVlrUnit := nPreco
	   _aVlrUnit[N] := nPreco
	ElseIf Empty(aCols[N][nCodPai])
       //If ValType(_nVlrUnit) == "N" .and. _nVlrUnit > 0 .and. Alltrim(ReadVar()) # "M->UB_DESC"
	   If _aVlrUnit[N] > 0 .and. Alltrim(ReadVar()) # "M->UB_DESC"
          //aCols[N,nPosUni] := _nVlrUnit
		  aCols[N,nPosUni] :=  _aVlrUnit[N]
	   Else
	      aCols[N,nPosUni]	:= aCols[n][nPosTab] - Round(((aCols[n][nPosTab] * aCols[n][nPosDes] /100)),4)
		  //_nVlrUnit := aCols[N,nPosUni]
		  _aVlrUnit[N] := aCols[N,nPosUni]
       EndIf
	   nPreco   := aCols[N,nPosUni]
	EndIf
	If Alltrim(ReadVar())=="M->UB_QUANT"
	   nQtde   := M->UB_QUANT
	Else
	   nQtde   := aCols[N,nPosQua]
	EndIf
...
If !Empty(aCols[N,nPosTab])
	If (nPreco > aCols[N,nPosTab])
		...
		aCols[N,nPosIte]   := A410Arred(nPreco*nQtde,"UB_VLRITEM")
//		aCols[N,nPosDes]   := 0
        aCols[N,nPosDes]   := aCols[N,nPPDesCom] // Desconto do prazo mantem cfe Ademir 
//		aCols[N,nPosVDe]   := 0
		aCols[N,nPosVDe]   := A410Arred(((aCols[N,nPosTab]*aCols[N,nPPDesCom])/100)*nQtde,"UB_VALDESC") // Calcular valor do desconto com base no desconto do prazo
	...
	ElseIf (nPreco < aCols[N,nPosTab])
	   ...
	   // aCols[N,nPPAcres]   :=  M->UA_X_ACRES        
		...
		//DO PEDIDO. 
        // If cEmpAnt == "02" .OR. cEmpAnt == "11" .OR. cEmpAnt == "12" .OR. cEmpAnt == "15" .OR. (cEmpAnt == "01" .AND. cFilAnt == "03") //SM0->M0_CODIGO == "02" .or. (SMO->M0_CODIGO =="01" .and. Alltrim(SM0->M0_CODFIL) == "03")
		...
		aCols[N,nPosVDe] 	:= A410Arred(((aCols[N,nPosTab]*nDesc)/100)*nQtde,"UB_VALDESC")
