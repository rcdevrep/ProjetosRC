...
/*/
User function envMailA(cEmail,cAssunto,cMensagem,nOpcao,cCCO,cBcc,aAtt,cFile,_cFrom)
	...
	#DEFINE EMAIL_PASSWORD         "4gr.C0p3lSaSt3mAs" //Alltrim(SuperGetMv("MV_RELAPSW",.F.,"123!@#as",))
	...
	Default cFile	:= ""
	Default aAtt	:= {}
	Default _cFrom    := EMAIL_ACCOUNT

	Conout('envMailA - inicio - '+cAssunto)
	...
	Else
		...
		EndIf
		// conout( oMail:GetErrorString( nret ) )
		Conout('envMailA - ERRO AO CONECTAR '+ oMail:GetErrorString( nret ))
		Return .F.
	Endif
	...
	Else
		...
		EndIf
		 conOut("envMailA - [ERROR]Falha ao autenticar: " + oMail:getErrorString(nRet))
	...
	If nRet == 0
		...
		oMailMessage:clear()
		oMailMessage:cFrom    := alltrim(_cFrom)//EMAIL_ACCOUNT
		...
		If nErr <> 0
			...
			oMailMessage:cTo := "suporte.sistemas@agricopel.com.br"
			oMailMessage:cSubject := "ERRO EMAIL-VERIFIQUE-("+alltrim(cEmail)+")- "+oMail:getErrorString(nErr)+cAssunto
			nErr := oMailMessage:send(oMail)
			oMail:smtpDisconnect()
		...
		Else
			...
			EndIf
		   cONOUT("envMailA -  Enviado E-mail para: "+Alltrim(cEmail))		
		Endif
	Else
		// Conout( nret )
		// Conout( oMail:GetErrorString( nret ) )
