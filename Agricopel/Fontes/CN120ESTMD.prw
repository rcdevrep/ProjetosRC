...
User Function CN121EST()
	...
	Local _cMailTI	:= SuperGetMv("MV_XERRO69",.F.,"sistemas@agricopel.com.br")
	...
	If _lEst
		If _cPlnLocV == CN9->CN9_TPCTO
			...
			xGeraMail("ESTORNO MEDICAO DE CONTRATO (CNTA121)",_cMailGCT,CN9->CN9_NUMERO,CND->CND_NUMMED,"a RECEBER (SE1)","Cliente: ",CNA->CNA_CLIENT," / Loja: ",CNA->CNA_LOJACL,Posicione("SA1",1,xFilial("SA1")+CNA->CNA_CLIENT+CNA->CNA_LOJACL,"A1_NOME"),CND->CND_NUMTIT,CND->CND_PARCEL,CND->CND_VLTOT,CNF->CNF_DTVENC,CN9->CN9_XTPPAG,CN9->CN9_XMSGPG)
			u_xHtmlPad("Medicao de Contrato Estornada",cMensg)
			u_envMailA(_cMailGCT,"GCT - Medicao de Contrato Estornada (CNTA121)",cHtml,2/*,_cMailTI,"carlos.s@agricopel.com.br"*/)
			...
			If _lGerSf2
				...
				If !Qry1->(Eof())
					...
					DbSelectArea("SD2")
					DbSetOrder(3)
					If DbSeek(xFilial("SD2")+Qry1->F2_DOC+Space(TamSX3("D2_DOC")[1]-len(Qry1->F2_DOC))+Qry1->F2_SERIE+Space(TamSX3("D2_SERIE")[1]-len(Qry1->F2_SERIE))+CNA->CNA_CLIENT+CNA->CNA_LOJACL+"000000000000001")
						RecLock("SD2",.F.)
						SD2->D2_ORIGLAN := "LF"
						MsUnlock("SD2")
					EndIf
					...
					Else
						...
						u_envMailA(_cMailTI,"GCT - Erro ao excluir NF Saida Manual (MATA920)",cHtml,2/*,"","carlos.s@agricopel.com.br"*/)
					EndIf
					ConOut("Fim  : "+Time())
					ConOut(Repl("-",80))
		...
		ElseIf _cPlnLocC == CN9->CN9_TPCTO
			...
			// Gera email de alerta
			xGeraMail("ESTORNO MEDICAO DE CONTRATO (CNTA121)",_cMailGCT,CN9->CN9_NUMERO,CND->CND_NUMMED,"a PAGAR (SE2)","Fornecedor: ",CNA->CNA_FORNEC," / Loja: ",CNA->CNA_LJFORN,Posicione("SA2",1,xFilial("SA2")+CNA->CNA_FORNEC+CNA->CNA_LJFORN,"A2_NOME"),CND->CND_NUMTIT,CND->CND_PARCEL,CND->CND_VLTOT,CNF->CNF_DTVENC,CN9->CN9_XTPPAG,CN9->CN9_XMSGPG)
			u_xHtmlPad("Medicao de Contrato Estornada",cMensg)
			u_envMailA(_cMailGCT,"GCT - Medicao de Contrato Estornada (CNTA121)",cHtml,2/*,_cMailTI,"carlos.s@agricopel.com.br"*/)
	...
	If !Empty(_cCodFor) .and. !Empty(_cLojFor)
		...
		If MsSeek(xFilial("SE2")+aDadosTit[03]+Space(TamSX3("E2_PREFIXO")[1]-len(aDadosTit[03]))+aDadosTit[04]+cParcela+_cTipoE2+Space(TamSX3("E2_TIPO")[1]-len(_cTipoE2))+_cCodFor+_cLojFor)
			...
			Else
				...
				u_envMailA(_cMailTI,"GCT - Erro ao excluir Contas a Pagar (FINA050)",cHtml,2/*,"","carlos.s@agricopel.com.br"*/)
			EndIf
			ConOut("Fim  : "+Time())
			ConOut(Repl("-",80))
