...
User Function MTA920C()       
	...
	If !ISINCALLSTACK("CNTA121")
		...
		If (SF2->(FieldPos("F2_PLACA")) > 0)
			...
			MV_PAR01 := xOldPerg1
		EndIf    
		 _cTes  := ascan(aheader,{|x|upper(alltrim(x[2]))=="D2_TES"})

		 For nX := 1 to len(Acols)
			TesGF:= Posicione('SF4',1,xFilial("SF4")+Acols[N][_cTes],"F4_DUPLIC")
			If TesGF == "S"
				lGF := .T.
				exit
			EndIf 
		...
		If lGF .AND. (cEmpAnt == "39" .OR. cEmpAnt == "01" .OR. cEmpAnt == "30" .OR. cEmpAnt == "22" .OR. cEmpAnt == "31" .OR. cEmpAnt == "41" .OR. cEmpAnt == "92" ;
			.OR. cEmpAnt == "50" .OR. cEmpAnt == "02" .OR. cEmpAnt == "03" .OR. cEmpAnt == "59" .OR. cEmpAnt == "43" .OR. cEmpAnt == "47" .OR. cEmpAnt == "34" )  //Somente para MCL e Agricopel Icara
...
Static Function PerMTA920(cPerg)
	Pergunte(cPerg,.T.)
Return()
