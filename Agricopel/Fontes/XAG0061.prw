...
Programa para simulação de Impostos 
...
User function XAG0061(cCliente,cLoja,cTipo,cProduto,cTes,nQtd,nPrc,nValor,aCab)
	...
	aImp[35] := "PS2"			            ; aCab[35] := 'PIS/Pasep - Via Apuração' 				//35 PIS/Pasep - Via Apuração
	aImp[36] := MaFisRet(1,"IT_BASEPS2")	; aCab[36] := 'Base do PS2 (PIS/Pasep - Via Apuração)' 	//36 Base do PS2 (PIS/Pasep - Via Apuração)
	aImp[37] := MaFisRet(1,"IT_ALIQPS2")    ; aCab[37] := 'Aliquota PS2 (PIS/Pasep - Via Apuração)' 		//37 Aliquota PS2 (PIS/Pasep - Via Apuração)
	aImp[38] := MaFisRet(1,"IT_VALPS2")	    ; aCab[38] := 'Valor do PS2 (PIS/Pasep - Via Apuração)' 		//38 Valor do PS2 (PIS/Pasep - Via Apuração)
	aImp[39] := "CF2"				        ; aCab[39] := 'COFINS - Via Apuração' 			//39 COFINS - Via Apuração
	aImp[40] := MaFisRet(1,"IT_BASECF2")    ; aCab[40] := 'Base do CF2 (COFINS - Via Apuração)' 		//40 Base do CF2 (COFINS - Via Apuração)
	aImp[41] := MaFisRet(1,"IT_ALIQCF2")    ; aCab[41] := 'Aliquota CF2 (COFINS - Via Apuração)' 		//41 Aliquota CF2 (COFINS - Via Apuração)
	aImp[42] := MaFisRet(1,"IT_VALCF2")	    ; aCab[42] := 'Valor do CF2 (COFINS - Via Apuração)' 		//42 Valor do CF2 (COFINS - Via Apuração)
...
//Relatorio com Simulação de Impostos
User Function XAG0061R()
    ...
    AAdd( aPergs ,{1,"Preço:",_nPrc,"@E 999,999.99",'.T.',,'.T.',50,.T.}) 
