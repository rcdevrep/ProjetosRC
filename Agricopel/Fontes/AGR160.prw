...
User Function AGR160()            
	...
	If cEmpAnt == "01" .OR. cEmpAnt == "16"
		//If ((SM0->M0_CODIGO == "01" .And. (Alltrim(SM0->M0_CODFIL) == "03" .or. Alltrim(SM0->M0_CODFIL) == "15" .or. Alltrim(SM0->M0_CODFIL) == "16")) .OR.;
		If ((SM0->M0_CODIGO == "01" .And. (SuperGetMV("MV_XRGDESC",.F.,.F.))) .OR.;
			(SM0->M0_CODIGO == "01" .And. Alltrim(SM0->M0_CODFIL) == "02" .And. M->UA_TABELA == "777"))   //Consistencia para nao repassar desconto para empresa Agricopel Base.
			M->UA_DESCCOM := 0	
			M->UA_X_ACRES := 0
		Else
