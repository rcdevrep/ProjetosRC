...
//AGRICOPEL INI AUMENTAR PARA NAO QUEBRAR NAS LINHAS DE PRODUTOS E SERVICOS
#DEFINE MAXITEMC   040 //042 //038                                                // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN  096//110//080                                               // Máximo de caracteres por linha de dados adicionais
//AGRICOPEL FIM	
#DEFINE MAXMSG     013					// Máximo de dados adicionais por página
//AGRICOPEL INI AUMENTAR PARA NAO QUEBRAR O VALOR																  
//#DEFINE MAXVALORC  009					// Máximo de caracteres por linha de valores numéricos
#DEFINE MAXVALORC  012  //009                                               // Máximo de caracteres por linha de valores numéricos
//AGRICOPEL FIM 		
...
If nTipo <> 1
	...
	Else
		...
		If lVerPerg
			if !lJob
				AJUSTASX1()//Específico Agricopel
...
Local cIndex	 := ""
Local cChave	 := ""
local cXml		:= ""// Agricopel - Retira acentos Atualizado pela TOTVS
Local lQuery     := .F.
Local nX         := 0
Local nI		 := 0
Local oNfe
...
Local cCampos	:= ""
Local lFirst 	:= .T.
local lChave	:= .F.// Agricopel - Retira acentos Atualizado pela TOTVS
Local cChavSF3	:= ""
Local nTotalReg := 0
...
If !lImpDir .or. MV_PAR04 == 0 /* Caso impressão de DANFE seja realizada via AutoDistMail */
	...
	#IFDEF TOP
		...
		//Específico Agricopel	- Inicio 
		If (!Empty(MV_PAR09))
			cWhere += " AND EXISTS (SELECT SD2.D2_DOC "
			cWhere += "             FROM " + RetSqlName("SD2") + " SD2 (NOLOCK) "
			cWhere += "             WHERE SD2.D2_DOC = SF3.F3_NFISCAL "
			cWhere += "             AND   SD2.D2_SERIE = SF3.F3_SERIE "
			cWhere += "             AND   SD2.D_E_L_E_T_ = '' "
			cWhere += "             AND   SD2.D2_FILIAL = '" + xFilial("SD2") + "'"
			cWhere += "             AND   SD2.D2_LOCAL = '" + AllTrim(MV_PAR09) + "') "
		EndIf
		// Agricopel Fim 
		cWhere += "%"
			...
			For nX := 1 To nLenNotas
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
								...
								If SF1->F1_FORMUL == "S"
									if RecLock("SF1")
										...
										If SF1->(FieldPos("F1_CHVNFE")) > 0
											...
											U_GOX1XMLM("<nfeProc>" + aXML[nX][2] + "</nfeProc>")
										EndIf
						...
						cAviso := ""
						cErro  := ""
						// Agricopel - Retira acentos Atualizado pela TOTVS							 
						//-----------------------------------------------------------------------
						...
						if type("oRetNF") == "U"
							cXml := removeCaracEspecial( aXML[nX][2] ) 
...
If Len(aGrvSF3)>0 .And. SF3->(FieldPos("F3_CHVNFE"))>0
	...
	For nI := 1 To Len(aGrvSF3)
		lChave := iif( lUsacolab, .T., Empty(SF3->F3_CHVNFE) )// Agricopel - Retira acentos Atualizado pela TOTVS																								
...
For nZ := 1 To nLenDet
	...
	//Agricopel 
	If SD2->D2_TP == 'CO'
		AADD(aAux, Alltrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1])))) // AGRICOPEL
		AADD(aAux, Alltrim(TransForm(nVTotal,TM(nVTotal,TamSX3("D2_TOTAL")[1])))) // AGRICOPEL
	Else
	...
	If SD2->D2_TP == 'CO'
			AADD(aAuxCom, Alltrim(TransForm(nVUnitTrib,TM(nVUnitTrib,TamSX3("D2_PRCVEN")[1])))) // AGRICOPEL
		Else
			AADD(aAuxCom, AllTrim(TransForm(nVUnitTrib,TM(nVUnitTrib,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2]))))
		Endif		
		If lUf_MG
...
//AGRICOPEL INI 
//COMENTADO FONTE PADRAO PARA REALIZAR A QUEBRA CONFORME SEPARACAO POR PONTO E VIRGURA
/*
...
If Type("oNF:_InfNfe:_infAdic:_infAdFisco:TEXT")<>"U"
	...
	While !Empty(cAux)
		nPosAux	:= At(";",cAux)
		If Empty(nPosAux) .or. nPosAux > MAXMENLIN
			cMsgAux := StrTran(SubStr(cAux,1,MAXMENLIN), ";", " ") 
			If alltrim(cMsgAux) <> "."
				aadd(aMensagem,cMsgAux)
			EndIF
			cAux := SubStr(cAux,MAXMENLIN + 1)
		Else  
			cMsgAux := StrTran(SubStr(cAux,1,nPosAux), ";", " ")
			If alltrim(cMsgAux) <> "."
				aadd(aMensagem,cMsgAux)
			EndIf
			cAux := SubStr(cAux,nPosAux+1)
		EndIf
	EndDo
EndIf
...
//AGRICOPEL INI
//TRATAMENTO DA QUEBRA DE LINHA NAS INFORMACOES COMPLEMENTARES UTILIZANDO COMO SEPARADOR PONTO E VIRGURA 
//COMENTADO FONTE PADRAO
/*
...
For Nx := 1 to Len(aMensagem)
	If ValType(aMensagem[Nx]) == "A" //Len(Alltrim(aMensagem[Nx])) == 2
		NoChar(aMensagem[Nx][1],lConverte)

	ElseIf Len(Alltrim(aMensagem[Nx])) > 0
...
EndIf
aAdd(aMensagem, SC5->C5_MENNOTA) //Ajuste mensagem na nota
...
//AGRICOPEL INI 
//NAO IREMOS INFORMAR COMPLEMENTO DO CLIENTE
//COMENTADO CODIGO PADRAO
...
For nX := 1 To Min(nLenMensagens, MAXMSG)
	if ValType(aMensagem[Nx]) == "A"  //Len(Alltrim(aMensagem[Nx])) == 2//aMensagem[nX][2]
		If aMensagem[nX][2]
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			If ValAtrib("oEmitente:_EnderEmit:_xCpl") <> "U"
				//AGRICOPEL INI
				//NAO IREMOS IMPRIMIR COMPLEMENTO DO CLIENTE
				//COMENTADO FONTE PADRAO 
			    /*cStrAux		:=	"Complemento: "+AllTrim(NoChar(oEmitente:_EnderEmit:_xCpl:TEXT,lConverte))
				nForTo		:=	Len(cStrAux)/40
				nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
			...
			IF MV_PAR05 <> 1 .And. nMensagem <= nLenMensagens
				...
				nLenMensagens:= Len(aMensagem)
				--nMensagem
				For nX := 1 To Min(nLenMensagens - nMensagem, MAXMSG) // AGRICOPEL INICIO
					/*
					...
					*/
					...
					if Len(aMensagem[nMensagem+nX]) == 2//aMensagem[nX][2]
						If aMensagem[nMensagem+nX][2]
							oDanfe:Say( nLin, 002, aMensagem[nMensagem+nX][1], oFont08N:oFont )
						Else
							oDanfe:Say( nLin, 002, aMensagem[nMensagem+nX][1], oFont08:oFont )
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If ValAtrib("oEmitente:_EnderEmit:_xCpl") <> "U"
			//AGRICOPEL INI 
			//NAO IMPRIMIR COMPLEMENTO DA EMPRESA
			//COMENTADO FONTE ORIGINAL
			/*	cStrAux		:=	"Complemento: "+AllTrim(NoChar(oEmitente:_EnderEmit:_xCpl:TEXT,lConverte))
			nForTo		:=	Len(cStrAux)/40
			nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
			For nX := 1 To nForTo
...
If lMensagens
	...
	For nX := nMensagem To nForMensagens // AGRICOPEL INI
		/*
		if aMensagem[nX][2]
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	*/		
	aTamCol := {50,;
				150,;
				34,;//AGRICOPEL - AUMENTAR CAMPO NCM
...
*/
	...
	For nZ := 1 To nLenDet
		...
		// Tratamento para quebrar os digitos dos valores
		aAux := {}
		AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2]))))// AGRICOPEL
			//Agricopel 
		If SD2->D2_TP == 'CO'
			AADD(aAux, Alltrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1])))) // AGRICOPEL
			AADD(aAux, Alltrim(TransForm(nVTotal,TM(nVTotal,TamSX3("D2_TOTAL")[1])))) // AGRICOPEL
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	//AGRICOPEL INI 
	//NAO IMPRIMIR COMPLEMENTO DO EMITENTE
	//COMENTADO FONTE PADRAO
/*cStrAux		:=	"Complemento: "+AllTrim(NoChar(oEmitente:_EnderEmit:_xCpl:TEXT,lConverte))
	nForTo		:=	Len(cStrAux)/40
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
...
/*/
//-----------------------------------------------------------------------
static function removeCaracEspecial( cTexto ) // Agricopel - Retira acentos Atualizado pela TOTVS
...
/*/
//-----------------------------------------------------------------------
static function validCaracter( cDados )// Agricopel - Retira acentos Atualizado pela TOTVS
...
/*/
//-----------------------------------------------------------------------
user function listCaracter()// Agricopel - Retira acentos Atualizado pela TOTVS
...
/*/
//-----------------------------------------------------------------------
user function listHTMLCaracter()// Agricopel - Retira acentos Atualizado pela TOTVS
...
//Ajusta perguntas na SX1 - Específico Agricopel
Static Function AjustaSX1()
	...
	//Grupo/Ordem/Pergunta/Per Esp/Per Ing/Var/Tipo/Tamanho/Decimal/Presel/GSC/Valid/F3/GrpSxg5/cPyme/cVar01
	AADD(aRegs,{cPerg,"09","Local do Produto (NF Saída) ?","","","MV_CH9","C",2,0,0,"G","","MV_PAR09","","","","","","","","","","","","","","","","","","","","","","","","","NNR","",""})
	...
	For i := 1 to Len(aRegs)
		If (!SX1->(DbSeek(cPerg+aRegs[i,2])))
			...
			For j := 1 to SX1->(FCount())
				If j <= Len(aRegs[i]) .and. !(left(alltrim(FieldName(j)),6) $ 'X1_CNT/X1_PRE')
