...
//³ ExpExcel                                                                  ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Method ExpExcel(_aArray,_cDestino) Class TUtil
...
Method ExecutaQuery(_cQuery,_cAlias,_lReturn,_cIdent,_lExpQuery) Class TUtil
	...
	If _lExpQuery
		If _cIdent != Nil
			U_SIDGERATXT(_cQuery,"c:\"+_cIdent+".txt")
		Else
			U_SIDGERATXT(_cQuery,"c:\testeQuery.txt")
		Endif
	Endif
...
Method registraAnaliseUso(_cFuncao) Class TUtil
	...
	If SuperGetMv("PR_LOGAUSO",.t.)
		...
		TCSpExec("SP_ANALISE_USO_CRIAR",If(_cFuncao!=Nil,_cFuncao,_cFunName),__cUserId,FwGetMnuFile(),cModulo,_cFunName)
...
Method registraWS(_cServico,_cMetodo,_cSessao,_cConteudo) Class TUtil
	...
	If SuperGetMv("PR_LOGAWS",.t.)
		
		TCSQLEXEC("INSERT INTO ANALISE_WS (SERVICO,METODO,SESSAO,DATA,HORA,CONTEUDO_ENTRADA) VALUES ('"+_cServico+"','"+_cMetodo+"','"+_cSessao+"','"+dtos(MsDate())+"','"+Time()+"','"+ StrTran(StrTran(_cConteudo,'"',"#||#"),"#|#","#")+"')")
